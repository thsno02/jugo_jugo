...
范围论证：类型化 wikilink 是一个独立的结构化机制，区别于普通链接和索引导航。它改变的是链接的语义密度而非链接的数量或方向，因此值得独立成卡。与 wiki-compounding-artifact（复利积累）相关但不重合——复利机制描述积累的内容类型，类型化链接描述积累中链接的语义质量。
