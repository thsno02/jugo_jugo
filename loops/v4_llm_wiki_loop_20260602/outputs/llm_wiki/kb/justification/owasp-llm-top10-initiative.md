...
card: ../cards/owasp-llm-top10-initiative.md
...
范围论证：该材料为 OWASP LLM Top 10 2025 版着陆页，实际抓取内容仅含一段概述性介绍，未包含具体漏洞列表（列表在 PDF 下载文件中）。本卡仅提取该段落中可用的原子声明：倡议性质（社区驱动）、启动时间（2023）、关注对象（AI 应用特有安全问题）、趋势判断（LLM 嵌入深度增加导致新漏洞不断被发现）。未注入任何外部知识。
