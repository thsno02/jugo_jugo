...
[^src-1]: `data/raw/arxiv/arxiv-lightmem/text.txt` -- Abstract -- "Large Language Models (LLMs) struggle to effectively leverage historical interaction information in dynamic and complex environments"
[^src-2]: `data/raw/arxiv/arxiv-lightmem/text.txt` -- Abstract -- "Memory systems enable LLMs to move beyond stateless interactions by introducing persistent information storage, retrieval, and utilization mechanisms"
[^src-3]: `data/raw/arxiv/arxiv-lightmem/text.txt` -- Abstract -- "existing memory systems often introduce substantial time and computational overhead"
[^src-4]: `data/raw/arxiv/arxiv-lightmem/text.txt` -- Abstract -- "reducing total token usage by up to 38x / 20.9x and API calls by up to 30x / 55.5x"
