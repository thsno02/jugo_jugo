...
summary: >-
  three-layer-architecture 是 LLM Wiki 的三层结构：不可变的原始资料层（source of truth）、
  LLM 拥有的 wiki 层（markdown 页面）、人机共同演化的 schema 层（配置约定）
related: []
---
...
**Schema 层**——一份配置文档（如 Claude Code 的 CLAUDE.md 或 Codex 的 AGENTS.md），告知 LLM wiki 的结构、约定和工作流程。这是关键配置文件——它使 LLM 成为有纪律的 wiki 维护者而非通用聊天机器人。用户和 LLM 随使用经验共同演化 schema[^src-3]。
...
[^src-3]: `data/raw/gist_raw/karpathy-gist-llm-wiki/text.txt` -- "Architecture > The schema" -- "it's what makes the LLM a disciplined wiki maintainer rather than a generic chatbot. You and the LLM co-evolve this over time"
