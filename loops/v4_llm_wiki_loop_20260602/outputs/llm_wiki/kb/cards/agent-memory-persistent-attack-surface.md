...
related: [cross-session-continuity, etamp-environment-memory-poisoning, memgpt-memory-hierarchy, rag-knowledge-corruption-attack, rag-knowledge-database-attack-surface, raw-vs-consolidated-memory-vulnerability]
---
...
[^card-4]: [MemGPT 两级内存层次结构](memgpt-memory-hierarchy.md) -- 本卡分析 agent 记忆的持久性攻击面属性，MemGPT 的 archival storage 提供了持久化存储的具体机制——投毒内容一旦写入即可跨会话驻留，且 LLM 自主检索行为使激活无需用户干预
