...
edited_entity: llm
source_ids: [obsidian-community-plugin, developersio-jp-pattern, atlan-llm-wiki-vs-rag-dynamic-20260524, robin-cartier-llm-knowledge-bases]
justification: ../justification/comparison-wiki-rag-positioning-spectrum.md
canonical_concept: wiki-rag-positioning-spectrum
aliases: [wiki-rag定位光谱, positioning spectrum, 反RAG到混合光谱]
summary: >-
...
related: [full-context-anti-rag, rag-wiki-complementarity, scenario-based-tool-selection, wiki-rag-hybrid-pattern]
  - full-context-anti-rag
  - rag-wiki-complementarity
  - wiki-rag-hybrid-pattern
...
**光谱位置二：功能互补。** 日本社区的实践发现，wiki 与 RAG 自然形成分工——临时查询用 RAG，全局理解用 wiki，两者可以并存[^card-2]。这一立场承认 RAG 在特定场景中有独立价值。

**光谱位置三：混合架构。** Atlan 的分析将互补关系系统化为三种架构模式：wiki 作为策展层锚定 RAG 检索、两层分离、受治理的元数据层[^card-3]。这一立场不仅承认 RAG 的价值，还认为 wiki 可以增强 RAG 的表现。

**光谱位置四：场景驱动选择。** Robin Cartier 引入关系型知识库作为第三选项，形成三段式选择框架：个人用 wiki、运营用关系型、企业用 RAG/混合[^card-4]。这一立场最为务实，将选择问题从「wiki vs RAG」转化为「何种场景用何种工具」。
