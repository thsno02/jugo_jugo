...
值得注意的是，虽然框架本身是自愿性的，但行政令 EO 14110 将其作为生成式 AI 概况文件的基础框架予以引用 [^src-2]，这在事实上提升了框架的权威性和实践影响力。[编者注]自愿性框架通过行政指令间接获得准规范地位，是治理工具演进中的常见模式。AI 600-1 正是在此基础上将通用框架原则具体化为生成式 AI 领域的跨行业风险管理指南 [^card-2]。
