...
related: [ingest-operation, dual-audience-artifact, three-layer-architecture]
---
...
这一发现对 LLM Wiki 的三层架构（raw sources 层）有直接的实践指导意义：raw sources 的组织粒度不仅是文件管理问题，更是影响整个 wiki 质量的关键设计决策。

## Footnotes

[^src-1]: `/Users/lw/Desktop/GitHub/llm_wiki/jugo_jugo/data/raw/hacker_news/hacker-news-original-thread/text.txt` -- vbarsoum 评论 -- "The naive version (each book as 1 file) produced exactly the slop people are describing here. But splitting into chapter-level files and recompiling changed the output categorically. Same model, same prompts — the only variable was source granularity."
[^src-2]: `/Users/lw/Desktop/GitHub/llm_wiki/jugo_jugo/data/raw/hacker_news/hacker-news-original-thread/text.txt` -- vbarsoum 评论 -- "The compiler produced 210 concept pages with 4,597 cross-references (19.2 avg links per page). 20+ concepts synthesized across all 3 books unprompted... 173K words of output from 155K input. It's not compression — it's synthesis."
