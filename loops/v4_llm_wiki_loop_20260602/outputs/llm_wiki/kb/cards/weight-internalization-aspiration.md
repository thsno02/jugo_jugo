...
aliases: [权重内化, knowledge internalization, weight-based knowledge, 权重内化愿景]
summary: >-
...
related: [index-based-navigation, llm-wiki-pattern, llm-wiki-scale-boundary]
---
...
这一愿景指向了上下文窗口知识与权重知识之间的根本区别：当前 LLM Wiki 的工作方式是将知识放在上下文窗口中供 LLM 查阅（通过索引文件和摘要实现导航），但这受限于上下文窗口的容量。[编者注]权重内化则意味着 LLM 不再需要"阅读"wiki 就能"知道"其中的内容，从根本上消除了规模瓶颈。
