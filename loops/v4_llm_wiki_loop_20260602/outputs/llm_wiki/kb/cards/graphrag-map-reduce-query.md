...
related: [graphrag-community-hierarchy, graphrag-global-sensemaking, sensemaking-vs-retrieval-query]
---
...
该 map-reduce 流程的设计动机直接对应于 sensemaking 查询的需求——这类全局性问题无法通过局部检索回答，必须聚合跨社区的信息[^card-1]。

## Footnotes

[^src-1]: `data/raw/arxiv/arxiv-graphrag/agent_source_bundle.txt` -- Section 1.1.6 (graph_rag.tex) -- "Given a user query, the community summaries generated in the previous step can be used to generate a final answer in a multi-stage process."
...
[^src-4]: `data/raw/arxiv/arxiv-graphrag/agent_source_bundle.txt` -- Section 1.1.6 (graph_rag.tex) -- "Intermediate community answers are sorted in descending order of helpfulness score and iteratively added into a new context window until the token limit is reached."
...
[^card-1]: [意义建构查询与检索查询的区分](sensemaking-vs-retrieval-query.md) -- 本卡描述回答 sensemaking 查询的 map-reduce 机制，该卡定义了此类查询的本质特征，构成解决方案与问题定义的互补
