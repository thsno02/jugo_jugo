...
这一论点为 LLM Wiki 模式提供了独立于理论优势的**实用性理据**：wiki 中间层不是对未来技术进步的权宜之计，而是对当前上下文窗口架构固有局限性的务实回应。WiCER 论文的实证研究从机制层面验证了这一社区观察——注意力稀释导致全上下文推理在知识规模扩大时性能退化至低于 RAG[^card-attention-dilution-at-scale]。
