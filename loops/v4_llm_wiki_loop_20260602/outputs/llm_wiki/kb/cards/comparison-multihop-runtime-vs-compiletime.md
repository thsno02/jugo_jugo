...
**运行时函数链（MemGPT 路径）**：代理在查询时动态发出多次函数调用，每次调用的结果作为下一次调用的输入。MemGPT 的嵌套键值检索实验展示了这一能力——GPT-4 基线在 3 层嵌套时准确率降至 0%，而 MemGPT + GPT-4 在所有层级上性能稳定[^card-1]。优势在于灵活性：无需预先知道哪些信息会被串联。劣势在于受底层模型代理能力约束（GPT-4 Turbo 反而比 GPT-4 表现更差）。
...
**核心区分**在于多跳依赖的解析时机：查询时（runtime）还是摄入时（compile-time）。这一区分映射到更广泛的系统设计权衡——即时灵活性 vs 预计算稳健性。GraphRAG 的 map-reduce 流程代表了第三条中间路径：在摄入时预建社区摘要（编译时），但在查询时通过 map-reduce 聚合这些摘要（运行时），兼具两者的部分特征。
