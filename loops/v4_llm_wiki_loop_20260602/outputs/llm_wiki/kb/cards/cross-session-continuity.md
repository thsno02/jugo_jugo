...
summary: >-
  cross-session-continuity 是 LLM Wiki 通过三个持久化组件（raw sources、wiki 文件、schema）
  加 log.md 时间线实现跨会话连续性的机制；schema 应编码足够意图供未来会话使用，
  但解释性上下文（为何做出特定编辑决策）会在会话边界丢失
related: []
---
...
材料未讨论哪些具体信息会在会话边界丢失（如 ingest 对话中的上下文、人类的偏好指引、正在进行的分析思路），也未讨论 schema + log 的组合是否足以实现无缝衔接。
