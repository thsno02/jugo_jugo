...
aliases: [query phase, wiki query, querying the wiki]
summary: >-
  LLM wiki query 阶段 query-phase：构建完成后直接查询 canonical wiki 而非 raw sources。模型在已积累和结构化的知识上推理，实现纯 RAG 在大规模语料上难以达到的分析深度。
related: [llm-wiki-ingestion-workflow, llm-wiki-vs-rag-reasoning-depth]
---
