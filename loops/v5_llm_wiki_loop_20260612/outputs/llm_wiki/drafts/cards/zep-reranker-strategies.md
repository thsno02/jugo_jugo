...
status: draft
card_type: mechanism
tags: [reranking, RRF, MMR, cross-encoder, retrieval]
...
aliases: [Zep reranker, episode-mentions reranker, node distance reranker, 重排序策略]
summary: >-
  Zep 支持多种 reranker：RRF（多结果融合）、MMR（多样性重排序）、episode-mentions
  reranker（按实体/事实在对话中被提及频率排序，频繁引用的信息更易获取）、node distance
  reranker（按图距离从中心节点排序，提供局部化上下文）、cross-encoder（LLM 生成相关性
  分数，精度最高但计算代价最大）。初始搜索追求高 recall，reranker 追求高 precision。
related: [zep-hybrid-retrieval-pipeline]
---
