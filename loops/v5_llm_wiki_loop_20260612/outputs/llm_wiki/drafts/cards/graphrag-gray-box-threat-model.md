...
tags: [graphrag, threat-model, gray-box, adversary, corpus-modification]
...
aliases: [GraphRAG threat model, gray-box adversary, 灰盒攻击者模型, Attack Model]
summary: >-
  GraphRAG gray-box threat model 定义灰盒攻击者：知道 GraphRAG 整体流程（分块-提取-建图-社区-摘要-检索）
  但无法访问已构建图或模型参数。攻击者仅能修改可信源语料的小部分（如 Wikipedia 编辑），不能注入新文档。
  TKPA 攻击者知道社区结构驱动答案；UKPA 攻击者仅知道系统从文本构建知识图谱。gray-box corpus modification。
related: [graphrag-knowledge-poisoning-attack-surface, targeted-knowledge-poisoning-attack, universal-knowledge-poisoning-attack]
---
