...
status: draft
card_type: mechanism
tags: [hot-cache, session-memory, obsidian, context-persistence]
...
aliases: [hot cache, hot.md, session memory, recent context cache]
summary: >-
  claude-obsidian 的 hot cache 机制通过 wiki/hot.md 文件实现跨会话记忆：
  每次会话结束时自动更新 hot.md 为最新 context summary（约 500 words）；
  下次会话启动时首先读取 hot.md 获得 full recent context，无需 recap。
  跨项目引用时也建议优先读取 hot.md。由 hooks.json 中 SessionStart/Stop hooks 驱动。
related: [claude-obsidian-knowledge-engine]
---
