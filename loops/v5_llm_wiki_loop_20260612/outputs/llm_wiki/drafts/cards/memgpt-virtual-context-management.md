...
tags: [llm-memory, context-window, os-inspired, paging]
...
aliases: [virtual context management, 虚拟上下文管理]
summary: >-
  MemGPT virtual-context-management 虚拟上下文管理 借鉴操作系统虚拟内存分页机制,
  通过在主上下文(prompt tokens)与外部存储之间进行数据页入页出(paging),
  为固定上下文窗口的LLM提供扩展上下文长度的错觉。该技术使LLM能处理远超其原生上下文限制的任务。
related: [memgpt-memory-hierarchy, memgpt-function-chaining]
---
