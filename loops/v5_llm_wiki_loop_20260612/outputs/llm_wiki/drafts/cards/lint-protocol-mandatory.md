...
tags: [llm-wiki, lint, maintenance, orphan-pages]
...
aliases: [lint protocol, weekly lint, lint 不可省略, quality-control protocol]
summary: >-
  Lint 协议不可省略 lint-protocol-mandatory 指即使 wiki 表面看起来整洁也必须定期执行 lint。作者在 pitfall #1 中跳过 lint 导致三个 orphan 页面（无 inbound links）和一个未标记矛盾堆积，造成约一周"wait what's the current view?"混乱。作者实践为每周日早 ~20 分钟。Rohit v2 称之为 quality-control protocol，GBrain 用夜间 Dream Cycle cron 自动化。
related: []
---
