...
tags: [answer-relevance, reverse-question-generation, embedding-similarity, rag-evaluation]
...
aliases: [Answer Relevance, RAGAS answer relevancy, 答案相关性指标, AR]
summary: >-
  RAGAS Answer Relevance 通过逆向问题生成 (reverse question generation) 计算：从答案生成 n 个潜在问题 q_i，
  用 text-embedding-ada-002 取 embedding 后与原问题 q 计算 cosine similarity，AR = (1/n) * sum(sim(q, q_i))。
  不评估事实性，惩罚不完整或含冗余信息的回答。WikiEval 准确率 0.78。
related: [ragas-framework-overview, ragas-faithfulness-metric, ragas-context-relevance-metric]
---
