...
tags: [wikibase, datatype, entity, system-defined, extensibility]
...
aliases: [Datatype, Wikibase Datatype, data type]
summary: >-
  Wikibase Datatype 是 Entity 子类型，以固定 IRI 标识，决定 Property Value 的格式和形状。
  Datatype 集合为系统定义，仅开发者可扩展（编辑者不能在站点上添加）。每种 Datatype 需软件
  专门处理（如 UI 区别）。多数 Datatype 非原始类型，具有内部结构（如地理坐标含经纬度）。
related: [wikibase-property, wikibase-property-weak-typing, wikibase-quantity-value, wikibase-time-value]
---
