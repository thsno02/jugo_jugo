...
tags: [claim-extraction, entailment, refchecker, fine-grained-evaluation]
...
aliases: [claim entailment, claim-level checking, 声明级蕴含检查]
summary: >-
  Claim-level entailment checking 是 RAGChecker 的核心机制，包含 text-to-claim extractor 将文本 T 分解为 claim 集合 {c_i}，以及 claim-entailment checker 判断 claim c 是否被 reference text Ref 蕴含（记为 c in Ref 或 c not-in Ref）。RAGChecker 实现中使用 Llama3-70B-Instruct 同时作为 extractor 和 checker，基于 RefChecker 框架。
related: [ragchecker-framework-overview]
---
