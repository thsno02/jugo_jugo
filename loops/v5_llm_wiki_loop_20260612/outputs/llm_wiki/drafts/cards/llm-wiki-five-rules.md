...
tags: [llm-wiki, rules, compile-first, writeback, wiki-before-rag]
...
aliases: [Five rules, 五条规则, LLM Wiki rules]
summary: >-
  LLM Wiki 五条规则 (llm-wiki-five-rules): (1) Compile-first 不只回答要写入 wiki；
  (2) Writeback is mandatory 每个决策必须回写 wiki；(3) Wiki before RAG ~100 docs 内直读无需向量库；
  (4) Obsidian is replaceable paradigm is not 引擎是 LLM+filesystem+markdown；
  (5) Ideas outrank Code wiki 决策比代码更有价值。强制性递进关系。
related: [llm-compilation-paradigm, raw-wiki-code-architecture]
---
