...
tags: [wikibase, entity, datavalue, IRI, identity, value]
...
aliases: [Entity vs DataValue, Value types, Entity identity]
summary: >-
  Wikibase Value 分为 Entity（有全局唯一 IRI，可作 Snak subject）和 DataValue（无 IRI，靠内容/哈希标识，
  不能作 Snak subject）。Entity 包括 Item/Property/Datatype；DataValue 包括 QuantityValue/TimeValue/
  StringValue 等。Wikidata 不打算对数据值本身（如数字/字符串）存储 Statement。
related: [wikibase-item, wikibase-property, wikibase-datatype]
---
