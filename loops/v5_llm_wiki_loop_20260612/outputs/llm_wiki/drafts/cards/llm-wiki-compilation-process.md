...
status: draft
card_type: mechanism
tags: [llm-wiki, compilation, synthesis, confidence-scores, cross-references]
...
aliases: [compile, wiki compilation, article synthesis, LLM 编译, 文章合成]
summary: >-
  wiki-compilation-process：raw sources 编译为 wiki/concepts/ topics/ references/ 三类合成文章，带交叉引用双向链接和置信度评分（high/medium/low），使用双链格式 [[wikilink]] + markdown link，每个输出基于所有已编译文章递增强化
related: [llm-wiki-immutable-raw-sources, llm-wiki-dual-link-obsidian, llm-wiki-confidence-scores]
---
