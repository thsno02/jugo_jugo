...
status: draft
card_type: mechanism
tags: [ppi, prediction-powered-inference, confidence-interval, rag-evaluation, statistical-method]
...
aliases: [PPI, prediction-powered inference, 预测驱动推断]
summary: >-
  ARES 使用 Prediction-Powered Inference (PPI) 结合少量人工标注（约 150-300 个 human preference validation set）和大量 LLM judge 预测，学习 rectifier function 校正模型偏差，构建比纯标注或纯模型预测更紧的 95% 置信区间。PPI 在 judge 精度下降时（如跨域场景）仍能缓解排序退化，是 ARES 提供统计保证的关键。
related: []
---
