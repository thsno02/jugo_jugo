...
tags: [langgraph, store, search, vector-similarity, filter]
...
aliases: [store search, store.search, content filter, vector similarity search, 向量相似度搜索]
summary: >-
  LangGraph store.search 支持两种搜索方式的组合：content filter（filter 参数，
  精确键值匹配）和 vector similarity（query 参数，需 IndexConfig 配置 embed 函数和 dims）。
  两者可在同一 search 调用中组合使用。
related: [langgraph-store-data-model, langgraph-store-index-config]
---
