...
tags: [long-term-memory, RAG, Chain-of-Note, reading-strategy, structured-format]
...
aliases: [Chain-of-Note, CoN reading strategy, CoN + JSON, extract-before-read]
summary: >-
  chain-of-note-json-reading 在 LongMemEval oracle 检索设定下，Chain-of-Note（CoN）加 JSON 结构化格式的读取策略相比直接自然语言读取最高提升 10 个绝对百分点（GPT-4o）。CoN 指示 LLM 先从每个记忆项中提取信息再基于笔记推理，有效将长上下文阅读分解为"复制重要细节"和"基于简洁笔记推理"两个更简单的子任务。不使用 CoN 时 JSON 格式不一致地优于自然语言格式，但使用 CoN 后 JSON 格式一致地有益于各种能力水平的 reader LLM。
