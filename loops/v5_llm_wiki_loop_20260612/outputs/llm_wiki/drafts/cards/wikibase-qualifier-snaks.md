...
status: draft
card_type: mechanism
tags: [wikibase, qualifier, snak, statement, context-annotation]
...
aliases: [qualifier Snaks, qualifiers, qualifier]
summary: >-
  Qualifier Snaks 是 Statement 中附加的 PropertySnak 列表，修饰 mainSnak 但不直接描述 subject。
  典型用途：时间范围（任期）、角色关系（饰演角色）、比例修饰（百分比）。Qualifier 可选省略。
  例如 Obama 担任参议员的时间段、Emma Watson 饰演的角色。
