...
tags: [llm-wiki, originals, verbatim, cognitive-shape, gbrain]
...
aliases: [originals/ folder, originals 文件夹, verbatim-only, do-not-rewrite, the language is the insight]
summary: >-
  Originals 文件夹 originals-folder-verbatim 专门存放用户原始思维捕获，规则为 verbatim-only 禁止 LLM 编辑。源于 GBrain 的设计（填补 v1/v2 均未处理的缺口）。作者在 pitfall #2 中因允许 Claude 平滑改写而丢失原始认知形态（"knowledge compounding ≠ knowledge hoarding"被改为"compound knowledge effectively"）。教训：the language is the insight，表达形式本身承载认知价值。通过 do-not-rewrite tag 和 schema.md 禁令实现保护。
related: []
---
