...
tags: [llm-wiki, architecture, separation-of-concerns]
...
aliases: [three-layer architecture, 三层架构, sources/wiki/schema]
summary: >-
  Karpathy 三层分离架构：sources/（用户源文档，只读）→ wiki/（LLM 生成的实体/概念/索引页面）→
  schema/（Wiki 结构配置，命名规范/模板/分类）。数据流为 ingest 从 sources 到 wiki，
  query/maintain 在 wiki 层，schema 与 wiki 共演化。插件不修改源文件。
related: [karpathy-llm-wiki-concept, wiki-page-generation-output]
---
