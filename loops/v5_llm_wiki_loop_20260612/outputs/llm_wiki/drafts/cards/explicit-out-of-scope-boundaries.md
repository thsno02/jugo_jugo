...
tags: [out-of-scope, design-boundary, embeddings, vector-search, ocr]
...
aliases: [still out of scope, 排除范围, 不实现功能]
summary: >-
  explicit-out-of-scope-boundaries llm-wiki-karpathy 显式排除五项功能：
  embeddings/vector search、database-backed indexing、rename tracking、
  built-in OCR/vision/PDF parsing、autonomous background agents。
  体现运行时只管确定性结构而将智能工作交给外部 agent 的设计哲学。
related: []
---
