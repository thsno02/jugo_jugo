...
tags: [poisonedrag, text-decomposition, adversarial-text, malicious-text-crafting]
...
aliases: [P=S+I decomposition, malicious text decomposition, 恶意文本双子文本拼接]
summary: >-
  PoisonedRAG 将恶意文本 P 分解为两个子文本的拼接 P=S⊕I: S 负责满足检索条件(retrieval condition)使 P 被检索出, I 负责满足生成条件(generation condition)使 LLM 生成目标答案。先通过 LLM 生成 I, 再据攻击设定(black-box/white-box)构造 S, 拼接后双条件同时满足。实验证明 S⊕I 完整体显著优于单独使用 S 或 I。
related: [poisonedrag-dual-condition-framework, poisonedrag-black-box-attack, poisonedrag-white-box-attack]
---
