...
tags: [llm-wiki, knowledge-compilation, persistent-wiki, anti-rag, incremental-build]
...
aliases: [persistent knowledge compilation, 持久知识编译, incremental wiki building, 增量构建持久 wiki, Karpathy LLM Wiki pattern]
summary: >-
  LLM Wiki 持久知识编译范式（persistent knowledge compilation）：区别于传统 RAG（每次查询从头检索和推导），
  LLM 增量构建并维护一个持久 wiki——知识编译一次并保持更新，而非每次查询时重新推导。基于 Karpathy 的
  llm-wiki.md pattern，实现为跨平台桌面应用（Tauri v2）并附加大量扩展。
related: []
---
