...
tags: [llm-wiki, enterprise, scalability, access-control, concurrency]
...
aliases: [LLM wiki limitations, wiki enterprise constraints, wiki 企业限制, markdown folder limitations]
summary: >-
  LLM wiki enterprise limitations 包含三项核心约束：(1) 规模限制——index 必须放入 context，上限约 50k-100k tokens；(2) 访问控制缺失——markdown 文件夹无原生 RBAC，任何有文件访问权的 agent 可读全部内容；(3) 并发冲突——多用户/多 agent 同时更新产生 race conditions 和 write conflicts，无事务型数据库支持。Karpathy 本人将方案显式限定于个人研究者，这些不是 bug 而是设计假设的后果。
related: [context-window-threshold-50k-100k, llm-wiki-three-folder-architecture]
---
