...
tags: [llm-wiki, zero-dependency, markdown-native, host-agent-tools]
...
aliases: [zero dependencies, no servers, markdown native, 零依赖]
summary: >-
  zero-runtime-dependencies 设计原则：无服务器无服务无遥测，仅使用宿主 agent 内置工具（文件读写 web fetch web search），插件本身就是 Markdown（命令定义 skills 参考文档），编译查询 lint 生成可离线，研究摄取需网络，可选集成 ask-grok-mcp 和 tobi/qmd
related: [llm-wiki-five-install-modes, llm-wiki-hub-architecture]
---
