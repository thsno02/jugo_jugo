...
tags: [llm-wiki, agent-memory, rohitg00, autonomous-agent, scaling]
...
aliases: [LLM Wiki v2, rohitg00 agent memory, agent-memory pattern, LLM Wiki v2 持久记忆扩展]
summary: >-
  rohitg00 发布的 LLM Wiki v2 在 Karpathy 原始概念基础上增加 agent-memory pattern——为自主编码 agent 提供持久记忆引擎。解决长期一致性（consistance à long terme）和规模扩展（scaling）问题，使 wiki 适合由自主 agent 持续自动维护。
related: [llm-wiki-three-layer-architecture, llm-wiki-ingestion-workflow]
---
