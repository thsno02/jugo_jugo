...
status: draft
card_type: mechanism
tags: [llm-wiki, lint, automated-repair, causality-order]
...
aliases: [Smart Fix All, 一键修复, causality-ordered fix, 因果序修复]
summary: >-
  Smart Fix All 按因果顺序执行批量修复：修复污染页面 → 补全别名 → 合并重复 →
  修复死链 → 链接孤儿页 → 扩展空页面。v1.9.0+ 引入。因果序确保每步前置条件已满足。
related: [semantic-tiered-duplicate-detection, wiki-page-aliases]
---
