...
tags: [poisonedrag, fact-verification, fever, broader-nlp-tasks]
...
aliases: [FEVER dataset attack, fact verification attack, 事实验证攻击]
summary: >-
  PoisonedRAG 的设计原则(retrieval + generation conditions)可扩展至问答之外的 NLP 任务。在 FEVER 事实验证数据集上, 攻击者可使 RAG 输出错误的验证结果(SUPPORTS/REFUTES/NOT ENOUGH INFO)。黑盒 ASR 0.97, 白盒 ASR 0.88, F1-Score 0.98-0.99。该结果表明知识腐蚀攻击对知识密集型 NLP 任务具有普遍威胁。
