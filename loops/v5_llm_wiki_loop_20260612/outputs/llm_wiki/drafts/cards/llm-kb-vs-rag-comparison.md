...
tags: [knowledge-management, rag, llm-kb, retrieval, wiki]
...
aliases: [RAG vs wiki, RAG とどう違うのか, LLM KB と RAG]
summary: >-
  LLM Knowledge Base 与 RAG 的核心差异: RAG 逐 query 检索碎片无持久结构; LLM KB 事前整理为持久 wiki 并通过 filing back 成长。Karpathy 表示 ~100 articles / ~400K words 规模下不必 fancy RAG。作者认为两者非二择一: ad-hoc 查询用 RAG, 全局理解用 wiki。llm-kb-vs-rag-comparison RAG wiki 比較
related: []
---
