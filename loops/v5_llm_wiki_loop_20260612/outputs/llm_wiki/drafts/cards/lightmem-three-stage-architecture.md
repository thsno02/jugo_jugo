...
tags: [memory-system, llm-agent, atkinson-shiffrin, lightweight]
...
aliases: [LightMem, LightMem architecture, 三阶段记忆框架, memory-augmented generation]
summary: >-
  LightMem 受 Atkinson-Shiffrin 人类记忆模型启发，将 LLM agent 外部记忆组织为三阶段：Light1 认知感知记忆（pre-compression + topic segmentation）、Light2 主题感知短期记忆（STM buffer + LLM summarization）、Light3 睡眠时更新的长期记忆（soft update + offline parallel update）。该架构在 LongMemEval 和 LoCoMo 上以 GPT-4o-mini / Qwen3 为骨干模型，相比 A-MEM 等基线提升 QA 准确率最高 7.67%，同时减少 token 消耗最高 117 倍、API 调用最高 310 倍。
related: []
---
