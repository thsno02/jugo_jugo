...
tags: [llm-wiki, archive, topic-lifecycle, quiet-preservation, context-filter]
...
aliases: [topic archive, archive lifecycle, quiet preservation, topics/.archive, wiki archiving]
summary: >-
  llm-wiki 的归档机制将整个 topic wiki 从 HUB/topics/<slug> 移动到 HUB/topics/.archive/<slug>，在 wikis.json 中标记 status: archived。归档是上下文过滤器而非删除：内容保留但从默认 query/compile/research/output/maintenance 工作流中隐藏。Deep query 可展示归档索引匹配但不引用为活跃证据。恢复（restore）操作为反向移动。不归档单独文件或项目本地 .wiki/。
related: [hub-topic-wiki-isolation, lint-as-schema-migration]
---
