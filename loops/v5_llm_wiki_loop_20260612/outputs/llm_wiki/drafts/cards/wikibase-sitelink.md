...
status: draft
card_type: mechanism
tags: [wikibase, sitelink, badge, cross-wiki, interlanguage-link]
...
aliases: [sitelink, site link, interlanguage link, badge]
summary: >-
  Wikibase Sitelink 将 Item 链接到外部 wiki 页面，主要用于 Wikipedia 跨语言版本关联。
  约束：一个 Item 可有多个 sitelink（指向不同 wiki）但同一 wiki 至多一个。
  Sitelink 可附带 badges 集合（如"featured article"），badges 本身由 Item 表示。
related: [wikibase-item, wikibase-entity-description]
---
