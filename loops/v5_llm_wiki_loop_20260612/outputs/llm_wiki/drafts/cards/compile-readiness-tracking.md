...
status: draft
card_type: mechanism
tags: [compile-readiness, multimodal, representation, workflow]
...
aliases: [compile readiness, compile_readiness, 编译就绪度, ready/partial/needs_representation]
summary: >-
  compile-readiness-tracking 编译就绪度追踪通过 kb_prepare_source_bundle 返回三种状态：
  ready（全部表示就位可编译）、partial（部分就位）、needs_representation（缺少必要中间表示），
