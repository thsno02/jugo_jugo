...
tags: [langgraph, store, inmemorystore, postgresstore, backend]
...
aliases: [InMemoryStore, PostgresStore, store backend, 存储后端]
summary: >-
  LangGraph 提供两种 store 后端：InMemoryStore（内存字典，用于开发测试）和
  PostgresStore（PostgreSQL 数据库，用于生产）。PostgresStore 需 from_conn_string + setup() 初始化，
  使用 context manager 管理连接。两者共享相同 put/get/search API。
