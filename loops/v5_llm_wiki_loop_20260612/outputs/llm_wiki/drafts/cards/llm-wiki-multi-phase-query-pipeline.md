...
tags: [retrieval, query-pipeline, llm-wiki, vector-search, graph-expansion, budget-control]
...
aliases: [multi-phase query pipeline, 多阶段检索管线, query retrieval pipeline, optimized query retrieval]
summary: >-
  LLM Wiki 多阶段查询检索管线（multi-phase query pipeline）：Phase 1 Tokenized Search（中文 bigram + 英文分词）→
  Phase 1.5 可选 Vector Semantic Search（LanceDB，recall 从 58.2% 提升至 71.4%）→ Phase 2 Graph Expansion（4-signal
  模型 2-hop 带衰减）→ Phase 3 Budget Control（60/20/5/15 wiki/chat/index/system 分配）→ Phase 4 Context Assembly（编号页面 + citation）。
related: []
---
