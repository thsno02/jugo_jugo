...
tags: [threat-model, agent-security, memory-attack, related-work]
...
aliases: [eTAMP vs prior work, 威胁模型对比, memory attack taxonomy]
summary: >-
  eTAMP 在 agent/memory 安全研究中独特定位：(1) 无需直接记忆访问（vs AgentPoison 需直接操纵知识库）；(2) 单用户记忆模型（vs MINJA/CrAIBench 假设用户间共享记忆）；(3) 交互式沙箱环境（vs InjecAgent/AgentPoison 离线数据集）；(4) 跨会话持久攻击（vs WASP/RedTeamCUA/WIPI 仅单会话）。eTAMP 是唯一同时满足交互式环境+跨会话的工作。先前最接近的 WASP 虽用交互环境但仅测 within-session 攻击。
related: []
---
