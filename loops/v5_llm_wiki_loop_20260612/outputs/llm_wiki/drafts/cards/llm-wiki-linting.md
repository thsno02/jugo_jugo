...
status: draft
card_type: mechanism
tags: [linting, data-integrity, llm-agent, health-check, knowledge-quality]
...
aliases: [LLM health checks, wiki linting, LLM lint, 知识库健康检查, wiki health check]
summary: >-
  llm-wiki-linting LLM Wiki Linting健康检查机制 LLM对wiki执行"健康检查"以增量提升数据完整性：
  发现不一致数据、用web搜索补充缺失数据(impute missing data)、发现概念间有趣连接、建议新文章候选。
  超越传统代码lint：能主动生成内容、执行创造性关联发现。LLM擅长建议进一步探索问题。
related: [llm-knowledge-base-workflow, wiki-compilation-by-llm]
---
