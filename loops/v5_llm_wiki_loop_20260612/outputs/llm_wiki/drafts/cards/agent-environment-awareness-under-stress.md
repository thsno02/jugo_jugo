...
tags: [agent-awareness, environment-diagnosis, chaos-monkey, llm-reasoning]
...
aliases: [agent awareness, Agent 环境问题感知, environment problem diagnosis, 环境异常诊断]
summary: >-
  eTAMP 实验用 GPT-5.4 作 LLM judge 评估 Agent 是否明确诊断环境问题。Chaos Monkey 下 GPT-5.2 显示最高环境问题意识(7.4%)，Qwen3-32B(2.1%)，GPT-5-mini(1.1%)。无 Chaos 条件下意识接近 0%（极少误报）。高置信度案例包括"typed text becoming garbled"、"site auto-transforming text into gibberish"等明确诊断。但未发现意识与任务成功率之间的一致关系。GPT-5.2 高意识同时关联更高 TSR 和更大攻击脆弱性。
related: []
---
