...
tags: [wikibase, WON, serialization, notation, BNF, examples]
...
aliases: [WON, Wikidata Object Notation]
summary: >-
  Wikidata Object Notation (WON) 是概念文档中使用的简单序列化语法，用于给出具体数据实例
  示例和描述到 JSON/RDF 的映射。明确不打算用于实际实现。特点：位置编码（省略属性名）、
  BNF 文法描述、与数据模型结构一一对应。
