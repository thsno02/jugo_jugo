...
tags: [scale-ceiling, deduplication, context-window, llm-wiki]
...
aliases: [scale ceiling, 200 pages limit, deduplication fragility, wiki scale limit]
summary: >-
  LLM wiki pattern 存在规模上限：约 200 页 / ~100K tokens（index + content），超出后 LLM 无法在上下文中容纳 index，需要 sub-wikis 或 retrieval layer。去重（deduplication）依赖 LLM 判断，规模增大后脆弱——无确定性防护时 wiki 积累近重复页面。时序信号弱（仅 last_updated），单用户默认（无访问控制、无 merge conflicts 处理、无审计追踪）。
