...
status: draft
card_type: mechanism
tags: [llm-wiki, knowledge-fusion, incremental-update, multi-source]
...
aliases: [Smart Knowledge Fusion, 智能知识融合, incremental merge, 增量合并]
summary: >-
  多源增量更新合并新信息而不引入冗余。重复 ingest 同一源执行增量更新（新信息合并至
  entity/concept 页面）。矛盾被保留并附带归因。reviewed:true 页面受保护不被覆盖。
  Summary 页面则重新生成。
related: [contradiction-state-machine, three-layer-wiki-architecture]
---
