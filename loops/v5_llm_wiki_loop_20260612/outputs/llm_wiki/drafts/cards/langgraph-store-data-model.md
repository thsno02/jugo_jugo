...
tags: [langgraph, store, namespace, key, json]
...
aliases: [LangGraph store, namespace, key, store data model, 命名空间-键模型]
summary: >-
  LangGraph store 以 JSON document 为存储单元，按 namespace（类似 folder，
  含 user_id/org_id 等标识）+ key（类似 filename）层级组织，
  支持跨 namespace 的 content filter 搜索。namespace 为 tuple 结构如 (user_id, application_context)。
related: [langgraph-long-term-memory-concept, langgraph-store-search-capabilities]
---
