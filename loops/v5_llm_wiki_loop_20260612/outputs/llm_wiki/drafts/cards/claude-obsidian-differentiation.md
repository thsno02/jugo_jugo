...
tags: [obsidian-ai, smart-connections, copilot, differentiation, contradiction-detection]
...
aliases: [claude-obsidian vs Smart Connections, claude-obsidian vs Copilot]
summary: >-
  claude-obsidian 相比 Smart Connections 和 Copilot 的独有差异化能力：
  auto-organize（自动创建 entities/concepts/cross-references）、
  contradiction flagging（[!contradiction] callouts with sources）、
  session memory（hot cache 跨会话）、vault maintenance（8-category lint）、
  autonomous research（3-round web research with gap-filling）、
  batch ingestion（parallel agents 多源并行）、visual canvas（via claude-canvas）。
  竞品定位为 chat interface，claude-obsidian 定位为 knowledge engine。
related: [claude-obsidian-knowledge-engine, claude-obsidian-hot-cache]
---
