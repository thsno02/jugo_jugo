...
status: draft
card_type: concept
tags: [langgraph, long-term-memory, agent, persistence]
...
aliases: [long-term memory, 长期记忆, LangGraph long-term memory]
summary: >-
  LangGraph long-term memory 使 agent 跨不同 conversation 和 session 存储与召回信息，
  区别于 short-term memory（仅限单个 thread）。构建于 LangGraph stores 之上，
  通过 create_agent 的 store 参数接入。
related: [langgraph-store-data-model, langgraph-store-backend-comparison]
---
