...
status: draft
card_type: 问题现象
tags: [knowledge-compilation, information-loss, compression-failure]
...
aliases: [compilation gap, 编译差距, knowledge compilation gap, wiki compilation failure]
summary: >-
  Compilation gap 指 LLM 编译器将原始文档蒸馏为 wiki 时的质量退化。编译器忽略压缩目标过度压缩(目标75%实际35.4%, 目标50%实际12.2%, 目标25%实际8.2%)，丢弃关键事实。盲编译 wiki 评分 2.14-2.32 vs FC raw 3.46(17 RepLiQA 主题, 6800 QA pairs), 灾难性失败率(score-1)达 53-60% vs 17%。根因为 compression compliance failure。
related: []
---
