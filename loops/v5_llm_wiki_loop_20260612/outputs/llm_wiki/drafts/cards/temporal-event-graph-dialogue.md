...
status: draft
card_type: method
tags: [temporal-event-graph, dialogue-generation, causal-chain, persona-grounded]
...
aliases: [temporal event graph, 时间事件图, event graph G, causal event timeline]
summary: >-
  temporal-event-graph-dialogue 时间事件图驱动对话生成方法。为每个 agent 构建包含最多25个因果链接事件的 temporal event graph G，事件跨度6-12个月。用 text-davinci-003 基于 persona 迭代生成（每批k=3），后续事件以先前事件为条件保持因果一致性。对话生成时 agent 条件于两次 session 间新发生的事件子集 {e in G | t_k^s < t_i^e < t_{k+1}^s}，注入长程时间叙事。
related: [locomo-dataset, locomo-human-machine-pipeline, locomo-event-summarization-degradation]
---
