...
tags: [ukpa, structural-impact-score, entity-fragmentation, semantic-closeness, graphrag]
...
aliases: [Structural Impact Score, I_score, UKPA scoring function, 结构影响评分]
summary: >-
  UKPA structural impact score I_score = alpha*S_entity + beta*S_relation + gamma*(1-S_vec) 作为代理评分
  在无法访问最终图的条件下估计候选改写的结构破坏力。S_entity 为原始/修改 chunk 实体集对称差，
  S_relation 为关系集对称差，S_vec 为嵌入余弦相似度。默认 (0.25,0.25,0.5) 优先语义接近性保证隐蔽。
  消融：等权 QA acc 0.55 vs 默认 0.50；单组分仅 0.70-0.75。编辑距离<=3 已达最大攻击效果。
related: [universal-knowledge-poisoning-attack]
---
