...
status: draft
card_type: mechanism
tags: [feedback-loop, human-in-the-loop, quality-control, llm-pipeline]
...
aliases: [rejection feedback, rejection feedback loop, olw reject, auto-block]
summary: >-
  olw 的拒稿反馈闭环：reject draft 时附带原因存入 state DB，下次 compile
  该 concept 时注入 prompt（PREVIOUS REJECTIONS），LLM 需针对性修正。
  同一 concept 连续 5 次 rejection 无 approval 则自动阻断 auto-block，
  排除后续编译直至 olw unblock。rejection feedback loop auto-block
  五次拒稿阻断。
