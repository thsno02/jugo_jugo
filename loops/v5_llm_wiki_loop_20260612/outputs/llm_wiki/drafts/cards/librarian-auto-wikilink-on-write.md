...
status: draft
card_type: mechanism
tags: [wikilinks, auto-linking, obsidian-compatible, markdown]
...
aliases: [auto-wikilinks, auto-linking, library_write auto-link, library_suggest_links]
summary: >-
  Librarian 的 library_write 工具在写入文件时自动扫描已有笔记标题及 frontmatter aliases，将匹配文本包裹为 [[wikilinks]]。仅在显式写入时触发，从不修改用户未要求写入的文件。跳过代码块、行内代码、URL、已有 wikilinks。链接使用规范文件名以在大小写敏感文件系统上正确解析。支持 [[Note Name|alias]] 格式。librarian auto-wikilink auto-linking library_write。
