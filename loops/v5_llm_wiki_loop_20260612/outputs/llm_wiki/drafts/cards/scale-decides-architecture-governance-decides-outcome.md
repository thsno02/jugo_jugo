...
tags: [data-governance, architecture-selection, scale, enterprise-knowledge]
...
aliases: [scale decides architecture governance decides outcome, 规模决定架构治理决定结果]
summary: >-
  scale decides architecture governance decides outcome 是材料的核心论断：规模决定选择 LLM wiki 还是 RAG（小规模 wiki 胜，大规模 RAG 胜），但无论哪种架构选择，最终结果取决于底层数据治理水平。未治理数据使两种方案都会失败。企业真正需要的不是更好的检索架构，而是有治理的知识底层(substrate)。
related: [rag-upstream-data-quality-dependency, llm-wiki-enterprise-limitations, data-catalog-as-enterprise-wiki]
---
