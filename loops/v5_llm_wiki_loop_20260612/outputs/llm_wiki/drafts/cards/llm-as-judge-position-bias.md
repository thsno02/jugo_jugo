...
tags: [llm-as-judge, position-bias, order-sensitivity, evaluation-pitfall]
...
aliases: [LLM judge position bias, LLM 评判顺序偏差, order bias in LLM evaluation]
summary: >-
  当使用 LLM 在多个候选答案中选择最佳答案时，答案呈现的顺序会影响结果（position bias）。
  Wang et al. (2023) 指出 "Large language models are not fair evaluators"——LLM 对候选的排列顺序敏感，
  需要在评测设计中控制此偏差。RAGAS 的指标设计避免了直接让 LLM 做 pairwise ranking。
related: [ragas-framework-overview, rag-evaluation-motivation]
---
