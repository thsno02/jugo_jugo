...
- 关系抽取微调
- synthetic data for KG extraction
summary: GraphRAG graphrag-relationship-fine-tuning 关系抽取微调使用 GPT-4o 合成训练数据的三步流程：随机采样新闻主题生成 mock 新闻→用 GraphRAG prompt 抽取实体关系→基于抽取结果重新生成文章确保所有实体关系均被提及。第二次生成的文章作为微调输入，抽取的关系作为标签。共产出 29059 篇文章及对应标签。使用 Azure
  ML Studio 进行微调训练。
related:
