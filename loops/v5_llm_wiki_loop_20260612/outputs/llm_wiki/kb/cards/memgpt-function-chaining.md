...
- heartbeat mechanism
- 心跳机制
summary: MemGPT memgpt-function-chaining 函数链 request_heartbeat 心跳机制允许LLM在单次用户交互中 连续执行多个函数调用而不将控制权交还用户。LLM在函数输出中附带request_heartbeat=true标志时, 系统将函数结果追加到主上下文后立即触发下一轮LLM推理; 不带该标志则yield, 等待下一个外部事件。该机制使多步检索(如翻页、跨文档聚合)成为可能。
related:
