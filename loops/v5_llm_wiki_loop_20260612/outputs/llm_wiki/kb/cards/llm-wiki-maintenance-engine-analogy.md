...
- LLM as programmer
- wiki as codebase
summary: 'LLM 作为维护引擎 (llm-as-maintenance-engine): Karpathy 将 LLM 定位为 maintenance engine 而非 retrieval layer，类比 Obsidian=IDE / LLM=programmer / wiki=codebase。知识管理的真正痛点不是思考而是重复性记账任务（交叉链接、更新摘要、追踪矛盾、保持结构一致），这些恰好是
  LLM agent 可以吸收的工作。'
related:
