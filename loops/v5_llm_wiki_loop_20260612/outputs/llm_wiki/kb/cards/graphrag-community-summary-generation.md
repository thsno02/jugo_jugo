...
- community report
summary: GraphRAG graphrag-community-summary-generation 社区摘要生成分两级策略：叶级社区按边的源目标节点度（prominence）降序排列，迭代添加节点描述、边描述和 claims 直到 token limit；高层级社区若元素摘要超出 token limit 则以子社区摘要（更短）替换元素摘要（更长）直到适配窗口。生成的报告包含 title/summary/impact
  severity rating/detailed findings。摘要独立可用于理解语料全局结构。
related:
