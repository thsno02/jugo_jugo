...
aliases:
- learning by building RAG
- borrowed understanding
- 从零构建 RAG
- 动手学习价值
summary: 据作者主张，从零走一遍 RAG 全管线（chunking→embedding→retrieval→generation）比任何教程或课程教得更多。未亲手构建过
  RAG 系统的 AI 工程师是在"operating on borrowed understanding"。llm.c 的真正价值不在答案质量而在构建过程的学习收获。
related:
