...
aliases:
- Wikibase design goals
- Wikibase requirements
summary: Wikibase 数据模型的六大设计目标：Coverage（覆盖 Wikipedia 重要数据）、Simplicity（不过度复杂）、 Extensibility（允许未来扩展）、Flexibility（支持多用途复用）、Exchange（系统外数据语义明确）、 Technical support（可在 JSON/RDF/OWL 中充分表示）。这些目标有时互相冲突，需平衡取舍。
related:
