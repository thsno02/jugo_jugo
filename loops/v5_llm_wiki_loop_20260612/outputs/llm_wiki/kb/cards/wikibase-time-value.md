...
- date value
- temporal precision
summary: Wikibase TimeValue 包含 time（类 ISO 8601 时间戳）、precision（0-14 整数，0=十亿年到14=秒）、 before/after（以 precision 为单位的不确定范围）、timezone（UTC 偏移分钟）、calendarmodel（显示用 日历 URI）。存储始终用儒略前推格里历，calendarmodel 指定显示日历。世纪/千年不严格对齐最高位。
related:
- wikibase-datatype
