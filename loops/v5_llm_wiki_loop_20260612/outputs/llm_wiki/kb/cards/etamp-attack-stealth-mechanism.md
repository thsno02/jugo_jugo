...
- attack stealth
- 攻击隐蔽性
- conditional trigger dormancy
- ASR_A
summary: eTAMP 通过条件触发器设计实现攻击隐蔽性：恶意指令在 Task A 期间保持休眠，仅当 Task B 的环境特征匹配触发条件时才激活。实验验证
  ASR_A（Task A 过早触发率）在所有模型和策略组合中接近 0%，仅有 Qwen3.5-122B Authority(0.35%) 和 Qwen3-32B
  Baseline(0.71%) 极少例外。这意味着用户在 Task A 完成后无任何异常迹象表明记忆已被污染，攻击时间分离使实时监控难以检测。
related:
