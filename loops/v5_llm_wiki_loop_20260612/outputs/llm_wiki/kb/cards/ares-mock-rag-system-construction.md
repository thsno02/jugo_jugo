...
- mock RAG systems
- pseudo RAG systems
- artificial RAG splits
summary: 为测试 ARES 区分细微性能差异的能力，论文构建模拟 RAG 系统：从 KILT/SuperGLUE 验证集创建 9 个不同 split，成功率从
  70% 到 90%（每级间隔 2.5%）。正例直接使用原始数据，负例通过随机采样同文档/随机文档的段落和答案生成。每个 split 代表一个已知性能的 mock
  RAG 系统，使研究者能精确计算排名相关性。
related:
