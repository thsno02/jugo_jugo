...
aliases:
- topic wiki isolation
- one topic one wiki
- hub registry
- topic sub-wiki architecture
summary: llm-wiki 采用 hub + topic sub-wiki 隔离架构。Hub 是纯注册中心（仅 wikis.json, _index.md, log.md, topics/），无内容目录。每个 topic wiki 拥有独立的 raw/, wiki/, output/, inbox/，支持 Obsidian vault 独立打开。设计目标是防止跨主题噪声污染查询空间。wikis.json
  使用可移植相对路径以支持 iCloud 跨机器共享。
related:
