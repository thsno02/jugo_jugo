...
- kb-compile
- Claude Code 知識管理実装
summary: '作者(クラスメソッド 森茂)基于 Karpathy LLM KB 概念的 Claude Code 实装: workspace/knowledge/
  (Raw) + CLAUDE.md (Schema) + workspace/wiki/ (Wiki) + Memory MCP (Mem0+pgvector)
  检索层。/kb-compile 命令执行编译, 支持 --all 和 --lint。kb-compile-implementation Claude-Code
  mem0 pgvector'
related:
