...
- 审计操作
- 结构压力测试
summary: AUDIT 是伴侣记忆框架的慢周期补偿操作，通过临时悬挂最高引力条目并观察查询性能来压力测试。 三种结果：性能退化则恢复并确认引力；性能不变则减少引力（死重）；性能改善则归档（主动干扰）。 AUDIT 不是真值修正机制，它测试高引力条目是否仍为代理当前运行的负载承载。 作为框架对 Kuhn 问题的主要防御——使累积异常债务在条目级别可见而非在维基级别隐性积累。 AUDIT 敏感度是关键开放问题——如果压力测试查询集狭窄或自确认，僵化存活。
related:
