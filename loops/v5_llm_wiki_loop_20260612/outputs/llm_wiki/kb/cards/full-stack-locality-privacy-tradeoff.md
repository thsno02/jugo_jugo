...
- full-stack locality
- 全栈本地化
- privacy-convenience tradeoff
- data sovereignty
- local-first AI
summary: 知识库工具在隐私-便利性频谱上分三档：Notion AI（零设置/数据在他方/检索不透明）、Obsidian+插件（本地存储/推理调外部API）、本地LLM
  wiki（全栈本地/高摩擦/低质量/无UI）。作者结论：敏感工作笔记用本地 wiki 不可替代，其余场景 Obsidian 更实用。
related:
- claude-obsidian-differentiation
- llm-personal-data-second-brain
- full-context-vs-rag
- llmc-setup-friction-three-walls
