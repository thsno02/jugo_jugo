...
summary: Graphiti episode subgraph 存储原始输入数据（message/text/JSON），作为非损失性数据源 供语义实体提取。每条消息携带 reference timestamp t_ref 用于解析相对时间引用。 Episodic edges 连接 episodes 到其引用的 semantic entity 节点，支持双向索引以实现 溯源（citation）和前向遍历。
related:
