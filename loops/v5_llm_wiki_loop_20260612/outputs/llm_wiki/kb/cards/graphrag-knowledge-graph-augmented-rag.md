...
- 图谱增强RAG
- Microsoft GraphRAG
summary: GraphRAG 是微软研究院提出的结构化层次化检索增强生成方法，通过从非结构化文本中提取知识图谱、执行层次社区检测、生成社区摘要，在查询时利用这些结构提供上下文，克服了 baseline RAG 在跨信息关联和全局语义概括两类问题上的不足。GraphRAG knowledge-graph hierarchical community structured RAG。
related:
