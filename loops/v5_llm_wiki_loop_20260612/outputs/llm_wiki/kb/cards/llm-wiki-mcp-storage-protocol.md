...
aliases:
- WikiStorage
- WikiStorage Protocol
- build_server
- LocalFilesystemStorage
summary: llm-wiki-mcp 通过 WikiStorage Protocol（Python Protocol 类）实现存储后端可插拔。 接口包含 read_page,
  write_page, list_pages, append_log, read_log, write_raw_file 六个异步方法。通过 build_server(storage=instance)
  组合根注入。默认实现为 LocalFilesystemStorage（由 CLI --wiki-root 参数构造）。可替换为 SQLite、Notion、 GDrive
  或 test fake。领域错误类型 WikiConflictError, WikiNotFoundError, WikiPermissionError, WikiPathError,
  WikiSchemaViolationError 可从包根导入。
related:
