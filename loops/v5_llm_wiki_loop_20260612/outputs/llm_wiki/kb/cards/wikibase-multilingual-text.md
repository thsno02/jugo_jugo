...
- MultilingualMultiTextValue
- multilingual text
summary: Wikibase 多语言文本三层结构：MonolingualTextValue（language+value，单语言单短语）、 MultilingualTextValue（MonolingualTextValue 列表，每语言至多一个，表示直接翻译关系）、 MultilingualMultiTextValue（MonolingualMultiTextValue 列表，每语言可多个短语，无跨语言对应关系）。
