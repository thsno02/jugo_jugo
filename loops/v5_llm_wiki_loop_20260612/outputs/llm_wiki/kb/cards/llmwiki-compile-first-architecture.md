...
- LLM Wiki pattern
- Karpathy LLM Wiki
summary: llmwiki 实现 Karpathy 的 LLM Wiki 模式：将原始源编译为持久化互链 wiki， 而非 RAG 式查询时检索。概念获得独立页面、页面互相 wikilink、 query --save 将回答写回 wiki 使后续查询上下文更丰富，知识随使用累积。 与 RAG 互补而非替代。
related:
