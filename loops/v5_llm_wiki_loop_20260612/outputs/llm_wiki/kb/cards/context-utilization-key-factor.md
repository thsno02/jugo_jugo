...
aliases:
- Context Utilization is Key
- context utilization 关键性
summary: 'RAGChecker 实验发现 context utilization 在所有 generator metrics 中与 overall F1 的相关性最强。同时 context utilization 在不同 retriever 之间相对稳定（如 GPT-4 的 CU: BM25 61.4 vs E5-Mistral 60.4），这意味着通过提升 retriever 可直接改善
  overall recall。该发现表明生成器充分利用检索上下文的能力是 RAG 系统性能的关键因素。'
related:
