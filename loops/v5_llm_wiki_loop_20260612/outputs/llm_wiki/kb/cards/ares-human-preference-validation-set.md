...
- ARES annotation requirement
- PPI labeled set
summary: ARES 的 human preference validation set 是约 150-300 个人工标注的 query-passage-answer 三元组，包含正负例，用于 PPI rectifier function 学习和 judge 训练的 early stopping。最低有效门槛约 150 个标注（低于 100 时 tau 急剧下降）；400 个进一步提升。相比传统标注方法（每系统
  150 个，共需 1350+），ARES 少用 78% 标注。此 set 需领域专家标注，是 ARES 的主要人力成本。
related:
