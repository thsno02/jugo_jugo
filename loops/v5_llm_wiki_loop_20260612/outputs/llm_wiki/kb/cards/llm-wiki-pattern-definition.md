...
- Karpathy LLM Wiki
- llm wiki
summary: LLM Wiki 模式由 Andrej Karpathy 于 2026 年 4 月提出，定义为持久的、复合的知识库（persistent, compounding knowledge base），由 LLM 维护。与 RAG 不同，模型写一个 wiki 而非每次重新检索派生答案，wiki 每次使用都变得更丰富。提出后一周内即有开源实现、YouTube 讲解和博客评论。
related:
- llm-wiki-vs-rag
