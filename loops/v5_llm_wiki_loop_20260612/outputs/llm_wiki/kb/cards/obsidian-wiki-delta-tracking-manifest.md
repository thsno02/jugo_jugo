...
- incremental ingest
- manifest
summary: obsidian-wiki 通过 .manifest.json 实现增量追踪（manifest-based-delta-tracking）： 记录每个已摄入源的路径、时间戳、产出了哪些 wiki 页。下次 ingest 计算 delta 只处理 新增或变化内容。跨项目使用时 wiki-update 通过 last_commit_synced 字段配合 git log 只处理增量 commit。避免每次重新摄入整个文档库。
related:
