...
- test-time cost
- OP-update
summary: LightMem 将记忆系统效率评估分为在线测试时成本和离线成本两个视角。在线成本仅计 Light1 预压缩 + Light2 STM summarization + Light3 软插入；离线成本额外包含并行更新（OP-update）。论文报告两种口径：合计（online+offline）用于公平对比基线系统（基线的更新全在在线完成）；纯在线用于评估用户感知延迟。纯在线口径下 LightMem
  效率优势更大：GPT 骨干 token 减少 31-106 倍、API 调用减少 17-159 倍；Qwen 骨干 token 减少 30-117 倍、API 调用减少 25-310 倍。
related:
