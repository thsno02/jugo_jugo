...
- knowledge compounding
- wiki write-back
summary: my-llm-wiki 的 write-back 机制通过 llm-wiki note "<insight>" 命令从 Claude Code sessions 将洞察写回知识图谱，实现 graph compounds over time 的复合增长。这体现 Karpathy 将 wiki 视为 persistent compounding artifact 的设计意图——知识随使用而积累，而非每次查询重新推导。
related:
