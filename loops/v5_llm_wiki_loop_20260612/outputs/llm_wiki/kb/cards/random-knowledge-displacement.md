...
- 知识置换
- 随机置换效应
summary: Random knowledge displacement 是 WiCER 中 pinning 已诊断事实消耗 token 预算从而置换其他未 pinned
  内容、在先前通过的探针上产生新失败的现象。这是 WiCER 改进在 1-2 次迭代后趋于平台期的主因，也是与 CEGAR 的关键分歧点(CEGAR 中精化不引入新反例)。消融实验显示随机
  pinning 甚至可能损害质量。
related:
