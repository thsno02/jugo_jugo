...
summary: ARES 定义 RAG 系统三维评估：context relevance（检索段落是否与 query 相关）、answer faithfulness（生成答案是否忠实于检索段落、无幻觉/外推）、answer
  relevance（答案是否与 query+段落相关）。每个维度由独立的二分类 judge 评估 query-passage-answer 三元组。此三维分解使开发者能定位
  RAG 管线中具体瓶颈组件。
related:
