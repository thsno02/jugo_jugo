...
- multi-Mac wiki
- iCloud 共享配置
summary: icloud-shared-hub-config：hub 可设为 iCloud Drive 路径使多 Mac 可见，config 存于 ~/.config/llm-wiki/config.json，共享 config 用逻辑 hub_path 非绝对用户路径，wikis.json 用相对 topic paths，macOS 隐私权限按 Mac 和启动进程隔离需分别授权
related:
