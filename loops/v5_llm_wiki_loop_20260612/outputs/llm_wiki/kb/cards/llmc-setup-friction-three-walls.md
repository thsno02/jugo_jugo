...
- macOS OpenMP issue
- llm.c compilation
summary: 搭建 llm.c 本地知识库的三大实操痛点：(1) macOS Clang 不支持 OpenMP 需通过 Homebrew 安装 GCC；(2)
  需将多文件笔记预处理为单个大文本文件，无内建 document loader；(3) CPU 推理慢（M2 MBP 30+秒/查询），CUDA GPU 可降至数秒。
related:
