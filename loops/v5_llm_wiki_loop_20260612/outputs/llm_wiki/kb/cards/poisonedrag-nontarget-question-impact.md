...
aliases:
- non-target question impact
- collateral damage
- 非目标问题影响
summary: PoisonedRAG 注入的恶意文本对非目标问题(non-target questions)影响极小。在 NQ 数据集上, 仅 0.3% (黑盒) / 0.9% (白盒) 的非目标问题检索到恶意文本, 且仅 0% / 0.4% 的非目标问题答案被影响。这意味着攻击具有高度针对性(targeted), 不会引起系统整体性能的可观测下降, 增强了隐蔽性。偶尔被检索的原因是恶意文本与非目标问题共享关键词(如均与
  Star Wars 相关)。
related:
