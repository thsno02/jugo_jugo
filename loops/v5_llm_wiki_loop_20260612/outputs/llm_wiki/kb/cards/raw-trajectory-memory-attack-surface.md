...
summary: 原始轨迹记忆(Raw Trajectory Memory/RTM)直接存储 Agent 完整的观察-动作对序列作为未来任务的上下文示例，不经 LLM
  摘要或整合处理。这使得 Agent 在网页中观察到的任何文本（包括恶意注入内容）原样持久化于记忆中，构成持久攻击面。区别于整合记忆(Consolidated Trajectory
  Memory/CTM)通过 LLM 摘要可能过滤恶意内容。eTAMP 论文聚焦 RTM 因其保留精确文本，直接支撑跨会话注入攻击。
related:
