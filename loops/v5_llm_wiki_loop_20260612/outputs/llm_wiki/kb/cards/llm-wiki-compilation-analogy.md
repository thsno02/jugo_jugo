...
aliases:
- source code / executable analogy
- code source / exécutable
- 源码与可执行文件类比
- knowledge compilation
summary: Karpathy 将 LLM wiki 类比为编译过程：源文档如源码（code source），wiki 如可执行文件（exécutable compilé）。知识预编译一次后反复查询，无需每次从原始材料重新推导。wiki 是活的编译产物，随新源增量更新，实现知识复合增长（compound）。
related:
