...
aliases:
- LLM wiki vs RAG
- wiki versus vector search
- LLM wiki 与 RAG 对比
summary: LLM wiki 适合个人级 10~数百文档的深度知识管理，优势在于多跳推理（multi-hop reasoning）和预合成知识；RAG 适合大规模非结构化语料库的即席检索。转折点在规模：超过数百文档需 vector database。两者非互斥，可混合部署。核心差异非速度而是推理深度。
related:
