...
- LLM Wiki Agent ingest
summary: LLM Wiki Agent 采用 compile-once-knowledge-management 架构：每次 ingest 是增量编译， wiki 是编译产物。源文档经 10 步 ingest workflow 转化为结构化 wiki 页面(source/entity/concept pages)， 同时更新 index、overview、标记矛盾、追加 log。与 RAG 的"每次查询重新推导"相对，
  知识只编译一次并持续维护。Knowledge graph 通过两遍构建(deterministic wikilink 解析 + semantic 推断) 生成 EXTRACTED/INFERRED/AMBIGUOUS 边，Louvain 聚类，SHA256 缓存增量处理。
related:
