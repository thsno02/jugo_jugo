...
- lost in the middle
- 8k context window
- GraphRAG context size
summary: GraphRAG graphrag-context-window-lost-in-middle 实验测试 8k 16k 32k 64k 四种 context window size 用于 gpt-4-turbo (128k max)。结果 8k 在 comprehensiveness 上普遍最优（平均 win rate 58.1%），diversity 和 empowerment 持平。论文将此归因于
  lost in the middle 现象（Liu et al. 2023）——LLM 在长上下文中间的信息召回降低。因此 GraphRAG 固定使用 8k token context window 进行社区摘要生成和查询回答。
related:
