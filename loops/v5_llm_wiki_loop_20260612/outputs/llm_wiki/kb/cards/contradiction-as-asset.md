...
aliases:
- contradictions are assets not errors
- 矛盾是资产而非错误
- contradiction protocol
- 矛盾标记协议
summary: '矛盾即资产原则 contradiction-as-asset 指 LLM wiki 中发现新旧声明矛盾时不覆盖而是标记保留两版。规则是"don''t
  overwrite, mark"——添加 contradicts: 字段，保留双方，在 lint 时浮现。作者在 pitfall #3 中违反此原则导致丢失历史推理（RAG
  vs LLM wiki 争论），两个月后需要旧论证时已不可恢复。Rohit v2 明确此规则而 v1 silent。'
related:
