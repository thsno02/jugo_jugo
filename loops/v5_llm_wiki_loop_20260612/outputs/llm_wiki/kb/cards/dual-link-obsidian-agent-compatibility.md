...
summary: llm-wiki 要求所有交叉引用在同一行使用双链接格式：[[slug|Name]] ([Name](../category/slug.md))。Obsidian
  读取 [[wikilink]] 用于图视图和反向链接；Agent 跟随标准 markdown 链接导航；GitHub 渲染标准链接为可点击。系统不锁定于任何工具——纯
  markdown 在任何文本编辑器中可读。See Also 链接必须双向（A->B 则 B->A）。
related:
