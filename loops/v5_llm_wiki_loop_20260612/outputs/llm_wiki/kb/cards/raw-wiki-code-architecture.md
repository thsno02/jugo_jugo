...
- raw wiki code layers
summary: 'Raw/Wiki/Code 三层架构 (raw-wiki-code-architecture): LLM Wiki 的文件系统结构。 raw 层存放不可变源材料(PDF/Excel/客户文档)；wiki 层是 LLM 编译产出的 markdown 当前共识， 持续更新；code 层是从 wiki 生成的执行制品(compiled artifact)而非真相。 ~100 docs /
  ~80k tokens 以内 wiki 可单次直读无需 RAG。支持 stale detection 和 delta-compile。'
related:
