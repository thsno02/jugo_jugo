...
- minority-pressure promotion
- variance preservation
- 少数假说保留
- 少数压力晋升
- 方差保持
summary: minority-hypothesis retention 少数假说保留是伴侣记忆框架的补偿机制， 休眠替代方案以低存储成本保存在缓冲区和隔离区中，使下一个巩固周期有历史方差可评分。 无此机制则少数立场必须在单一周期窗口内从零积累全部支持。有此机制则三个周期前开始 积累缓冲区压力的少数立场可在当前周期完成积累。 少数分支的生命周期：open -> promoted（集群越过晋升阈值）或
  closed（AUDIT 确认incumbent 仍为负载承载 且分支跨定义周期数未增长）。分支永远不被静默关闭。 受控方差注入借鉴推荐系统和强化学习的标准模式，目标不同（认知方差 vs 参与方差）但机械操作相同。
related:
