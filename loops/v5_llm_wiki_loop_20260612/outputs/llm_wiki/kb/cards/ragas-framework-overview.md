...
- ragas
summary: RAGAS (Retrieval Augmented Generation Assessment) 是一个 reference-free 的 RAG pipeline 自动评测框架， 提出三维度指标 Faithfulness / Answer Relevance / Context Relevance，通过 LLM prompting 实现无需 ground truth 的评测；在
  WikiEval 数据集上与人工判断一致性优于 GPT Score 和 GPT Ranking 基线。
related:
