...
aliases:
- llm-wiki-kit MCP tools
- wiki MCP server
- wiki_ingest wiki_search
summary: 'llm-wiki-kit 通过 MCP 协议暴露 8 个工具给 agent: wiki_ingest（处理 PDF URL YouTube markdown
  源）、wiki_write_page（创建或更新页面）、wiki_read_page（读页面）、 wiki_search（SQLite FTS5 全文搜索）、wiki_lint（断链孤儿矛盾检查）、wiki_status（页面数
  源数近期活动概览）、wiki_log（追加操作日志）、wiki_graph（交互式 HTML 知识图谱）。 通过 llm-wiki-kit serve 命令启动
  MCP server，兼容 Claude Desktop Codex Cursor Windsurf 等 agent 的 MCP 配置格式。'
related:
