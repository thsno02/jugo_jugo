...
aliases:
- lost in the middle
- context scaling limitations
- 上下文扩展递减收益
- 注意力分布不均
summary: 'context-scaling-diminishing-returns 上下文扩展递减收益: MemGPT论文指出直接扩展transformer 上下文长度面临二次计算成本增长(self-attention), 且即便成功扩展, 长上下文模型也难以有效利用 额外上下文 -- Liu et al.(2023)发现注意力分布不均匀(lost in the middle), 模型更能recall窗口首尾而非中间信息。这一双重局限性(计算代价+利用效率)
