...
- deprecated rank
- best rank
summary: Wikibase Statement 三级 Rank：Preferred（最重要/最新，默认展示）、Normal（正确但不宜默认全部展示）、 Deprecated（不可靠/已知错误）。"Best rank"机制：若存在 Preferred 则 best=Preferred，否则 best=Normal。 设计故意简洁——三级对应不同的数据访问、UI 展示和导出处理。
related:
