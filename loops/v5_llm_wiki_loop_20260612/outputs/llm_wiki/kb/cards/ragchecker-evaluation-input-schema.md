...
aliases:
- RAGChecker input format
- checking_inputs.json
- RAGChecker data format
summary: RAGChecker 评估输入为 JSON 格式，顶层 results 数组包含多个 query 对象， 每个 query 包含 query_id
  query gt_answer response retrieved_context 字段， 其中 gt_answer (ground truth answer)
  是唯一必需的人工标注，retrieved_context 为检索块列表含 doc_id 和 text。该格式要求使用者提供完整的 RAG pipeline 输出（检索结果+生成回答）以及参考答案。
related:
