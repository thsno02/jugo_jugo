...
- LLM Wiki pattern
- Karpathy's LLM Wiki
summary: Karpathy LLM Wiki pattern 由 raw/ 不可变源文件夹、wiki/ LLM 生成页面、CLAUDE.md schema 组成；LLM 循环执行编译/反向链接/矛盾检测/健康检查四项维护操作；类比 Obsidian=IDE LLM=programmer wiki=codebase；核心创新在 maintenance loop 而非 search
related:
