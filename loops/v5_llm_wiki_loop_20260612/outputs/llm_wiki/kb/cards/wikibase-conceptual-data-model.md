...
- Wikibase data model
- Wikibase DataModel
- Wikibase conceptual model
summary: Wikibase 数据模型（Wikibase DataModel）是概念层规范，回答"需要支持哪些信息"而非实现或序列化问题。 序列化（JSON/RDF）和内部数据结构由独立文档规定。UML 类图描述结构但不要求实现照搬。 模型可扩展但在任一时刻应记录系统中所有可能存储的信息。
related:
- wikibase-design-goals-requirements
- wikibase-won-notation
---
