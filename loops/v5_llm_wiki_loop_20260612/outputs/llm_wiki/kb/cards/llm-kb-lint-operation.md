...
- wiki lint
- ヘルスチェック
summary: 'LLM Knowledge Base 的 Lint 操作: wiki 全体健全性检查。LLM 检测矛盾数据、过时主张、孤立页面、缺失链接并提案修正。核心洞察——LLM 不会对保守任务(相互参照/一貫性チェック/統合更新)感到退屈而放弃。llm-kb-lint-operation lint 健全性チェック'
related:
