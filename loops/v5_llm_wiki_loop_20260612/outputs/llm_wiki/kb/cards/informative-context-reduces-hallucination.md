...
aliases:
- Informative Context Improves Faithfulness
- 信息丰富上下文降低幻觉
summary: 'RAGChecker 实验表明当 retriever 提供更多信息量（E5-Mistral 的更高 claim recall）时，所有 generator 的 faithfulness 提升、hallucination 和 self-knowledge 均降低。GPT-4: faithfulness 87.9→92.9，hallucination 8.7→5.7，self-knowledge
  3.4→1.4（BM25→E5-Mistral）。这表明 generator 有能力识别并利用上下文中的信息，当有更多可用信息时会减少对内部知识的依赖。'
related:
