...
- retriever metrics
- 检索器指标
summary: RAGChecker retriever metrics 包含两个指标：Claim Recall = |{c^(gt)_i | c^(gt)_i ∈ {chunk_j}}| / |{c^(gt)_i}|，衡量 ground truth claims 被 retrieved chunks 覆盖的比例；Context Precision = |{r-chunk_j}| / k，其中 relevant
  chunk 定义为包含至少一个 ground truth claim 的 chunk，衡量检索结果中有用 chunk 的占比。Context precision 采用 chunk-level 而非 claim-level，因为固定大小分块策略下 chunk 可能同时包含相关和无关信息。
related:
- ragchecker-framework-overview
- claim-level-entailment-checking
- ragchecker-overall-metrics
- claim-recall-eli5-evaluation
- chunk-overlap-minimal-effect
