...
- you own vs LLM owns
- llm-wiki 职责边界
summary: llm-wiki-human-machine-division 人机职责分工：用户负责(own)素材输入(sourcing raw material)、 提出问题(asking good questions)、方向引导(steering direction)、对错误提反馈(filing feedback)； LLM 负责(own)所有写作、交叉引用、归档、簿记(bookkeeping)及处理反馈(acting
  on feedback)。
related:
