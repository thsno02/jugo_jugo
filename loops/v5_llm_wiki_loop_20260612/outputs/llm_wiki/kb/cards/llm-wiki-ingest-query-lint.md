...
- operating loop
- 操作循环
summary: 'LLM Wiki 三种核心操作 (llm-wiki-ingest-query-lint): Ingest 读取新源→讨论→写摘要→更新索引→触碰相关页→追加日志；Query 针对 wiki 回答问题并可选归档分析为新页面；Lint 定期检查矛盾/过时声明/孤立页面/弱交叉引用/缺失概念。配合 index.md（内容地图）和 log.md（时间线记录）两个特殊文件。'
related:
