...
- Light1
summary: LightMem Light1 的预压缩子模块使用 LLMLingua-2（轻量 BERT 架构，<2GB GPU 显存）对原始对话 token 执行二元分类保留/丢弃决策。压缩率 r 为保留比例（0.4-0.8），阈值 tau 设为留存概率的第 r 百分位。实验表明 r 在 50%-80% 时压缩后内容的 QA 准确率与未压缩可比。该模块还支持基于交叉熵的生成式 LLM 替代方案——高条件熵
  token 语义独特性更强因此被保留。
related:
