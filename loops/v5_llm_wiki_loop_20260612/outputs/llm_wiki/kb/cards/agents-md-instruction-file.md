...
- agents.md
- instruction file
- wiki instruction file
summary: agents.md 指令文件 instruction-file：定义页面命名规则、新实体创建 vs 更新已有实体的判断标准、矛盾格式化方式。是保持 wiki 长期一致性的主要杠杆 (main lever)，其质量直接决定知识库可靠性。材料建议从项目开始就创建。
related:
- llm-wiki-three-layer-architecture
- llm-wiki-model-quality-risk
- llm-wiki-agent-config-as-program
