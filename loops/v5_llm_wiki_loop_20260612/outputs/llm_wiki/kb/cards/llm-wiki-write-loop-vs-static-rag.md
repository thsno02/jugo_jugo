...
- lint pass
summary: LLM Wiki 与传统 RAG 的核心区别是 write loop：LLM 自己撰写维护 wiki、构建 backlinks、将输出回归系统，语料库动态演进而非静态。lint pass 执行不一致性审计、补全缺失数据、建议连接，更接近助理维护 zettelkasten 而非搜索引擎返回 top-k chunks。反对观点认为检索最相关知识片段的基本问题本质仍是 RAG。
related:
- dual-audience-wiki-artifact
