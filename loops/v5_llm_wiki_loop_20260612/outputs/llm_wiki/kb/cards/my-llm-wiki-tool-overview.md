...
- llm-wiki CLI
- my_llm_wiki
summary: my-llm-wiki 是一个 Python 包（pip install my-llm-wiki），实现 Karpathy LLM Wiki 三层架构，将任意文件夹转为可查询知识图谱。使用
  Tree-sitter AST 提取 19 种编程语言的结构信息，Docling 处理 PDF/DOCX/PPTX/HTML/EPUB，输出为 Obsidian
  vault。SHA256 cache 实现增量更新。作者 phuc-nt，MIT 许可，Python >=3.10，v0.9.0 Beta。
related:
