...
- post-hoc citation
- 后置引用
- closed-book citation gap
summary: ALCE 实验发现 ClosedBook+PostCite 策略 (closedbook-posthoc-citation-gap) 呈现正确性与引用质量的系统性鸿沟。ClosedBook 在 ELI5 上正确性达 18.6%（优于 Vanilla 的 12.0%），但 citation recall 仅 15.5%，远低于 Vanilla 的 51.1%。在 ASQA 上，PostCite
  的 citation recall 比 Vanilla 低 47 个百分点（26.7% vs 73.6%）。原因有二：(1) 开卷模型易被无关段落干扰降低正确性；(2) ClosedBook 常生成正确但与检索段落不相似的文本，难以后置匹配引用。
related:
