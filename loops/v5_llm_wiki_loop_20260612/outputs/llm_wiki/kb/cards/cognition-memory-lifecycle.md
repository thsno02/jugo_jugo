...
aliases:
- Evidence Consolidation Decay Teaching
- Cognition memory lifecycle
- agent memory four stages
summary: Cognition agent-memory-lifecycle 四阶段：Evidence（prompts/files/tool calls/decisions/outcomes 成为 typed learning events）→ Consolidation（原始 trace 压缩为含 steps/checks/failure modes 的人类审批 skill）→ Decay（recall
  和 freshness 随时间建模，过时指导可刷新）→ Teaching（下一个 agent 获得路由：哪个 skill、谁的判断、如何应用）。person-specific retrieval 保留作者归因和品味。
related:
