...
- claim-level recall
- RAG F1
summary: RAGChecker overall metrics 在 claim 级别计算 precision 和 recall。Precision = |{c^(m)_i | c^(m)_i ∈ gt}| / |{c^(m)_i}|，即 response claims 中被 ground truth 蕴含的比例。Recall = |{c^(gt)_i | c^(gt)_i ∈ m}| / |{c^(gt)_i}|，即
  ground truth claims 中被 response 蕴含的比例。F1 为二者调和平均。这组指标面向 RAG 用户提供单一数值排名能力。
related:
