...
- 95% token reduction
- token efficiency
- LLM wiki token savings
- token 效率
summary: LLM wiki token efficiency 据 MindStudio 分析，在小规模个人知识库（约 100 篇文章、约 40 万词）下，相比 naive full-document loading 可减少 token 消耗达 95%。此优势在面对优化后 RAG pipeline 时收窄，超出单个 context window 后完全消失。对比基准非优化 RAG 而是"将全部源文档整体加载"。
related:
