...
aliases:
- GPT-4 labels
- GPT-4 vs human labels
- GPT-4 annotation replacement
summary: ARES 探索用 500 个 GPT-4 生成标注替代 human preference validation set。结果：GPT-4 标注使
  Kendall's tau 在多数场景下降 0.05-0.30，但将标注成本从数百个人工标注降至不到 10 个 few-shot prompt。PPI 效力随
  GPT-4 标注数量增加而持续改善。据论文推测，此方向对降低评估成本有潜力。
related:
