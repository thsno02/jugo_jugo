...
- 多模态摄入模型
summary: representation-first-design 表示优先多模态设计要求非文本资产（PDF/images） 必须先经过中间表示（OCR text/vision/metadata）存入
  .llm-kb/representations/， 才可编译为最终 source note。运行时不自行执行 OCR/vision，而是为 agent 提供 canonical
  存储位置并在 lint 时验证 review trail。文本/结构化数据仍可直接编译。
related:
