...
- RAG 管道
- 向量检索管道
summary: RAG three-stage pipeline 包含：(1) 文档切分为可检索片段(chunking)，(2) embedding model 将片段转为向量表示，(3) 向量存入向量数据库(Pinecone/Weaviate/pgvector)建索引。查询时：用户问题向量化→top-K 相似度检索→片段作为 context 交给 LLM 合成回答。chunking 策略直接影响检索质量，embedding
  漂移需持续维护。
related:
