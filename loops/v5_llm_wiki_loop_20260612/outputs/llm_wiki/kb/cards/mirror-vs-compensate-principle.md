...
aliases:
- mirror-vs-compensate
- mirror where mirroring serves utility compensate where mirroring damages it
- 镜像补偿原则
summary: mirror-vs-compensate principle 镜像与补偿原则是伴侣记忆系统的核心设计规则， 系统在操作维度（工作语境、负载承载结构、自引用连续性、词汇）上镜像用户， 在认知失败维度（僵化、证据压制、单一文化收敛）上补偿用户。 该原则为时间结构化程序冲突规则：流式路径默认镜像，计划整合窗口补偿，AUDIT 为慢周期仲裁者。 先行文献 "To Mask or to Mirror"
  (Qian et al.) 已命名此张力，本框架贡献的是跨时间尺度的具体解决程序。
related:
- companion-memory-system-class
- circularity-as-thesis
