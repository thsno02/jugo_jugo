...
- Promise.allSettled
summary: 并行页面生成：可配置 1-5 并发（默认 3），2-3 倍加速。使用 Promise.allSettled 隔离错误—— 单页失败不影响其他页面，失败页以指数退避重试。Rate Limit Guardian 自动检测限速并建议 降低并发/增加延迟/切换提供商。Smart Batch Skip 跳过已处理文件。
related:
