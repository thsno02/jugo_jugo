...
- ~100 articles
- ~400K words
summary: Karpathy 报告 LLM Knowledge Base 在 ~100 articles / ~400K words 规模下 LLM 自维护
  index+summary 可行, 不必 fancy RAG。但材料指出数千件/数百万語时可能需要补充机制。当前验证仅限 small scale。llm-kb-scale-threshold
  規模 閾値 スケール
related:
