...
- three-step workflow
- Pick-Drop-Query
summary: LLM Wiki 三步工作流：(1) Pick Your Schema 从模板选择或自建 schema.md；(2) Drop in Your Sources
  将原始文件放入 raw/ 文件夹并指示 LLM compile；(3) Query & Compound 添加新源后 LLM 更新已有页面而非创建孤立页，wiki
  逐周增强。
related:
- llm-wiki-pattern-definition
- llm-wiki-schema-templates
- llm-knowledge-base-workflow
- rag-hands-on-learning-value
---
