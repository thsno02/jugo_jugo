...
aliases:
- Cognition skill loop
- agent skill reuse workflow
- Ask-Capture-Save-Retrieve
summary: Cognition agent-memory-skill-loop 四步工作流：(1) Ask first — agent 任务开始时查询团队已有 skill；(2) Capture work — 命令/文件编辑/卡点/outcomes 成为 evidence；(3) Save skills — 系统生成 SKILL.md 草稿并等待人类 approve（confirm-first
  capture）；(4) Retrieve later — 后续 agent 遇同类问题自动加载已批准 skill。核心设计：人类审批门控、作者归因、可执行指导。
related:
