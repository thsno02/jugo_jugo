...
- knowledge compilation
- 编译一次持续维护
- compile-once maintain-forever
summary: llm-wiki 的核心架构范式：知识被编译一次、持续维护，而非每次查询从原始文档重新推导。AI agent 在 ingest 阶段将原始素材分析后生成结构化
  wiki 页面（实体页、主题页、摘要），查询时直接读取已编译产物。区别于 RAG 的每次查询时检索并推导模式。原始素材不可变存于 raw/，编译产物存于 wiki/。源自
