...
- initial setup
- 5-minute setup
summary: LLM wiki 搭建步骤 setup-procedure：5 分钟完成，无需开发技能。步骤：复制 Karpathy GitHub Gist → 粘贴到 LLM agent (Claude/Codex/Grok) → 创建空目录 → Obsidian 打开为 vault → 子文件夹放 raw sources → LLM 执行首次 ingestion。Obsidian 非必须，替代方案有
  VS Code + Markdown Preview Enhanced、Logseq。
related:
