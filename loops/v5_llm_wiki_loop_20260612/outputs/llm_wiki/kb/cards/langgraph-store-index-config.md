...
- index config
- 索引配置
summary: IndexConfig 是 LangGraph store 启用向量搜索的配置对象，需提供 embed（嵌入函数， 接收 Sequence[str] 返回 list[list[float]]）和 dims（向量维度）两个参数。 通过 InMemoryStore(index=IndexConfig(...)) 或 PostgresStore.from_conn_string(...,
  index=IndexConfig(...)) 传入。
related:
