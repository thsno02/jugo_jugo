...
aliases:
- compiled artifact
- compiled executable analogy
- source code vs executable
summary: LLM wiki 编译产物类比 compiled-artifact-analogy：Karpathy 将 raw sources 比作源代码，LLM wiki 比作编译后的可执行文件。不必每次查询都重新编译——wiki 保持 canonical、alive，并随每次新源 ingestion 而改善。与 RAG 的 re-derivation 形成对比。
related:
