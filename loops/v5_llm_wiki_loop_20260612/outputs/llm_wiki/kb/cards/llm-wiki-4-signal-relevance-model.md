...
summary: LLM Wiki 四信号相关性模型（4-Signal Relevance Model）：Direct link ×3.0（wikilinks 直接链接）、
  Source overlap ×4.0（共享同一原始来源，权重最高）、Adamic-Adar ×1.5（共享邻居按邻居度加权）、 Type affinity ×1.0（同类型页面加分）。用于知识图谱可视化和查询检索的
  graph expansion 阶段。 附带 Louvain 社区检测和 Graph Insights（surprising connections + knowledge
  gaps）。
related:
