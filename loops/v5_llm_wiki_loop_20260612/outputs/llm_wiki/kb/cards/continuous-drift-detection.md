...
- 持续健康检查
summary: 企业 LLM wiki 将 Karpathy 的按需健康检查升级为持续自动漂移检测；当新 PR 与 runbook 矛盾时系统检测冲突并草拟更新路由给 document owner 审批；检测节奏为周检而非季度审计以匹配企业变更速率；LLM 执行与 Karpathy vault 相同的一致性检查但自动化运行
related:
