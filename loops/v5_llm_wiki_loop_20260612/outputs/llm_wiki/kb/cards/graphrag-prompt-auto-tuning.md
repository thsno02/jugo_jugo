...
summary: GraphRAG 提供 prompt auto-tuning 功能，通过加载输入文本、切分为小 chunk（默认200 token）、采样文本单元（random/top/all/auto 四种选择策略）、运行 LLM 推断域信息和实体类型，生成领域适配的 extract_graph/summarize_descriptions/community_report 三个 prompt 文件。auto
  策略对文本嵌入降维后选取质心的 k 近邻。建议在正式索引前执行以提升图提取质量。prompt auto-tuning domain adaptation selection-method。
related:
