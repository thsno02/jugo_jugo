...
summary: Wikibase QuantityValue 包含 amount（decimal 主值）、可选 lowerBound/upperBound（不确定区间）、 unit（IRI 或"1"表无单位）。不确定区间语义故意未精确规定（可能是硬限或概率区间）。 单位用完整 IRI 表示以避免歧义，通常指向 Wikidata Item 或标准词汇表如 QUDT。
related:
- wikibase-datatype
- wikibase-time-value
