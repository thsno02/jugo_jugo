...
- Targeted KPA
- 定向知识投毒
summary: Targeted Knowledge Poisoning Attack TKPA 利用图论分析（中心性、社区结构、ego-subgraph）定位知识图谱中 结构上最脆弱的区域，选择性重写少量文本 chunk，实现对特定 QA 输出的精确操纵。四步流水线： Vulnerable Community Localization -> Ego-subgraph Extraction -> Chunk
  Scoring and Selection -> LLM-driven Manipulation。 平均 ASR 91.27%，修改量 <0.06% 语料。定向知识投毒攻击 TKPA graph-theoretic。
related:
- graphrag-knowledge-poisoning-attack-surface
- tkpa-vulnerability-score
- tkpa-chunk-scoring-function
- graphrag-defense-evasion
- graphrag-gray-box-threat-model
- tkpa-experimental-results
---
