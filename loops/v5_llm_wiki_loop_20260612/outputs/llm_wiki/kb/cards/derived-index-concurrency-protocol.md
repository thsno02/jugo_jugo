...
- concurrent-safe wiki
- _index.md cache protocol
summary: llm-wiki 的 _index.md 文件是派生缓存而非数据源（source of truth）。过期检测：比较目录内 .md 文件计数与索引表行数，不匹配则内联重建。这使得多个 Claude Code 会话可无锁并发写入同一 wiki——两次重建从相同磁盘文件派生，结果收敛至相同正确状态。log.md 采用 append-only 小原子写入，天然并发安全。
related:
