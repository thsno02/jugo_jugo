...
- Zep
- Zep memory layer
- Zep AI memory service
summary: Zep 是一种基于时序知识图谱的 AI Agent 记忆层服务，核心引擎为 Graphiti。其知识图谱 G=(N,E,phi) 包含三层子图：episode subgraph（原始消息）、semantic entity subgraph （提取的实体与事实）、community subgraph（聚类摘要）。采用 bi-temporal model 双时间线 追踪事实的现实有效期（T）与系统记录期（T'）。在
  DMR benchmark 上 94.8% vs MemGPT 93.4%， 在 LongMemEval 上准确率提升最高 18.5% 且延迟降低约 90%。
related:
