...
aliases:
- context as scarcest resource for AI agents
- agent context problem
- AI agent 上下文稀缺
summary: Anthropic 工程团队将 context 描述为 AI agent 最稀缺资源 agents need just-in-time access to current accurate context；AI coding agent 读取过时内部文档则产出过时输出；企业 LLM wiki 通过 Claude MCP 协议为 agent 提供持续维护的知识图谱查询接口；agent
  与人类查询同一图谱确保 grounded in same current context
related:
