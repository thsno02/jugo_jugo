...
summary: raw-wiki-schema-vault-structure 三层 Vault 结构将知识库分为 raw/（原始素材）、 wiki/（sources/outputs/concepts/entities/syntheses/_indexes/index.md/log.md）、
  .llm-kb/（manifest.json/runs.jsonl/representations/）三层， 体现 runtime-owned structure
  与 agent-owned synthesis 的分离。
related:
