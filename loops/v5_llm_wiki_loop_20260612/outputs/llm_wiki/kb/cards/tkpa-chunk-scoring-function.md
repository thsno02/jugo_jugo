...
aliases:
- Chunk Selection Score
- C_score
- ChunkScore
- 文本块选择评分
summary: TKPA chunk scoring function C_score = w1*S_graph + w2*S_semantic + w3*S_attitude 融合三个信号 对候选 chunk 排序。S_graph 基于 ego-subgraph 内实体 PageRank 中心性，S_semantic 为 chunk 与查询 嵌入余弦相似度，S_attitude 量化情感极性。默认权重
  (0.5, 0.3, 0.2) 优先图结构。 消融显示 graph 结构贡献最大；k=3 chunks 即达 91.2% ASR 饱和。Chunk Selection Score PageRank cosine similarity。
related:
