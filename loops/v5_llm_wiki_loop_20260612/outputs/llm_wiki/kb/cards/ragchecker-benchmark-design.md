...
- RAGChecker evaluation benchmark
- 10 域基准
summary: RAGChecker 配套基准涵盖 10 个领域（Wikipedia/Novel/Writing/Biomedical/Finance/Lifestyle/Recreation/Science/Technology/AI Science）共 4162 个 query，源自 RobustQA、KIWI、ClapNQ、NovelQA 等公开 ODQA 数据集。关键设计：将原始短答案通过
  GPT-4 转为长文本答案以匹配现代 LLM-based RAG 输出，并用 RefChecker 质控确保无幻觉。每条样本为 <query, documents, ground-truth answer> 三元组。
related:
