...
- Karpathy LLM Wiki
- wiki-memory pattern
summary: 'LLM Wiki pattern 是 Karpathy 2026 提出的三层架构: raw sources -> compiled wiki -> structured schemas。核心思想是知识编译一次后持续维护而非每次查询重新派生。实现方式为将编译后文档集加载至模型上下文窗口并持久化 KV cache 状态(如 llama.cpp --cache-prompt)，后续查询以亚秒延迟服务。与
  RAG 的根本区别在于消除了查询时检索步骤和检索失败风险。'
related:
