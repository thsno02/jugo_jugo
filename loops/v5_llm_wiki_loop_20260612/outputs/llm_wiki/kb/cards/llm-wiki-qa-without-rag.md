...
aliases:
- no RAG needed
- index files instead of RAG
- LLM自维护索引
- auto-maintaining index files
summary: llm-wiki-qa-without-rag LLM Wiki问答无需RAG的索引策略 在约100篇文章/40万词规模下， LLM自动维护索引文件和文档简短摘要即可支持复杂查询，无需向量检索(RAG)。 LLM能较好地读取所有重要相关数据。作者原以为需要fancy RAG但实际不需要。 隐含规模上限：作者用"at this small scale"限定，暗示更大规模可能需要不同方案。
related:
