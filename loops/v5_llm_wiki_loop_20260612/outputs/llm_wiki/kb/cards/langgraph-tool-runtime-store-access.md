...
- tool store access
- Tool 中访问 Store
summary: 'LangGraph agent tool 通过 ToolRuntime 参数访问 store。Tool 函数声明 runtime: ToolRuntime[Context] 参数， runtime.store 为传给 create_agent 的同一 store 实例，runtime.context 提供运行时上下文（如 user_id）。 context_schema 为 dataclass
  定义，调用 agent 时通过 context= 传入。支持读取（store.get）和写入（store.put）。'
related:
