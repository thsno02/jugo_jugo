...
aliases:
- knowledge database attack surface
- RAG 新攻击面
- knowledge corruption attack
summary: RAG 系统的知识库(knowledge database)引入了一个新的、实际可利用的攻击面(attack surface)。攻击者可向知识库注入少量恶意文本(malicious texts)，使 LLM 对特定目标问题生成攻击者指定的错误答案。攻击途径包括恶意编辑 Wikipedia 页面、发布虚假新闻或托管恶意网站、内部人员注入等。PoisonedRAG 是首个利用该攻击面的
  knowledge corruption attack。
related:
