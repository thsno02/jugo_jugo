...
- 检索不解决维护
- semantic search over stale content
summary: Glean Notion-AI Confluence-search 等企业检索工具只解决找到已捕获内容而不解决底层维护问题；语义搜索指向过时知识库返回自信但错误答案
  better retrieval over bad context delivers wrong answers more quickly；LLM Wiki 有趣之处在
  maintenance loop 非 search layer
related:
