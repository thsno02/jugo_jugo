...
- output tables
- parquet outputs
summary: GraphRAG Knowledge Model 定义了索引输出的数据抽象层，包含 7 种表：Document（输入文档）、TextUnit（分析切片，含 n_tokens/entity_ids/relationship_ids）、Entity（title/type/description/frequency/degree）、Relationship（source/target/description/weight/combined_degree）、Covariate（可选
  claims，含 subject/object/status/时间范围）、Community（Leiden 层次结构含 parent/children/level）、Community Report（title/summary/full_content/rank/findings）。所有表含 UUID id 和 human_readable_id。默认输出为 Parquet 文件。knowledge
  model schema entities relationships communities text-units parquet。
related:
- graphrag-six-phase-indexing-pipeline
- graphrag-community-hierarchy-leiden
---
