...
- ARES judge training
summary: ARES 使用 DeBERTa-v3-Large (304M) 作为 judge 基座，以对比学习目标在合成三元组上微调三个独立的二分类 judge，分别评估 context relevance、answer faithfulness、answer relevance。训练使用 cross-entropy loss + Adam、5e-6 学习率、batch size 32、early
  stopping（3 epoch 无改善）。设计目标是摆脱外部 API 依赖，可部署在商用 GPU 上。
related:
