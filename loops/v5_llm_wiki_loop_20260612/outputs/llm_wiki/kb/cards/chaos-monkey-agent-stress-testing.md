...
summary: 受 Netflix Chaos Monkey 启发，eTAMP 论文提出针对 Web Agent 的环境压力测试方法。在 Task B 执行期间对
  Agent 动作施加概率性变换：Click Drop(p=0.4 点击变空操作)、Scroll Swap(p=1 方向反转)、Type Transform(p=1
  Caesar 密码替换文本)。模拟现实网络延迟、UI 不响应、键盘故障等不确定环境。最大步数从 15 增至 37 以保证任务仍可完成但显著困难化。Chaos Monkey
  使平均步数翻倍，大部分模型 TSR 下降。
related:
