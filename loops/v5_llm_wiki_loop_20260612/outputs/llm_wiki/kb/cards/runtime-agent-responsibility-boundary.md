...
aliases:
- runtime owns vs agent owns
- 运行时职责
- agent 职责
- runtime philosophy
summary: runtime-agent-responsibility-boundary 运行时与智能体职责边界明确划分： runtime 负责 canonical
  paths/IDs、validation、deterministic writes、manifest tracking、wiki navigation； agent
  负责 summarization、OCR/vision/profiling、synthesis、分类决策、持续改进。 kb_prepare_source_bundle
  是两层之间的桥梁接口。
related:
- raw-wiki-schema-vault-structure
- representation-first-design
- source-id-repair-mechanism
---
