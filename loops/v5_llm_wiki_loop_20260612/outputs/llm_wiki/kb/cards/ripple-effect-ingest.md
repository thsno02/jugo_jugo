...
summary: Ingest 涟漪效应 ripple-effect-ingest 指每次向 LLM wiki 注入新文章时 Claude 会触及多个现有页面：添加
  backlinks、更新概念索引、标记矛盾、精炼 TL;DR。作者报告中位数每次 ingest 触及 9 个文件（范围 4-23），基于最近 30 次 ingest
  的 log.md 统计。此为 Karpathy 原始 gist 所称的 ripple effect 的实证验证。
related:
