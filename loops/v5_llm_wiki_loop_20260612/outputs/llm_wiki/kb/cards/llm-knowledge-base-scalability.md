...
- 合成数据微调
summary: llm-knowledge-base-scalability LLM知识库规模瓶颈与微调方向 当前LLM wiki工作流在约100篇文章/40万词的
  "小规模"下有效，暗示上下文窗口为扩展瓶颈。Karpathy提出的突破方向：合成数据生成+微调， 使LLM将知识编码到权重(weights)而非依赖上下文窗口(context
  windows)。 自建搜索引擎CLI亦为应对规模的中间方案。
related:
