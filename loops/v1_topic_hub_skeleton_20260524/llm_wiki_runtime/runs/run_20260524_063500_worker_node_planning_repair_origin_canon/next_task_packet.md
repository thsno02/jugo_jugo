...
handoff_from:: run_20260524_063500_worker_node_planning_repair_origin_canon
repairs_handoff_from:: run_20260524_063000_worker_node_planning_origin_canon
source_task_packet:: .llmwiki/runs/run_20260524_063500_worker_node_planning_repair_origin_canon/task.md
...
Generation must not write, update, or adopt `nodes/20260524_062000_llm_wiki_origin_and_canon/node.yaml`.

The root `nodes/<node_id>/node.yaml` is adopted metadata. It may be created only by a later adoption step after citation/adoption audit passes for `versions/1.0`.

Generation must not write `kb/`, `generated/`, or any adopted view files.
...
- Bundle path gate: all four artifacts are written under `nodes/20260524_062000_llm_wiki_origin_and_canon/versions/1.0/`.
- Adoption boundary gate: root `nodes/20260524_062000_llm_wiki_origin_and_canon/node.yaml` is not written during generation.
...
Generator should write `LOOP_DONE` only after all four version bundle artifacts exist under `nodes/20260524_062000_llm_wiki_origin_and_canon/versions/1.0/` and satisfy the gates. Otherwise write `LOOP_BLOCKED` with the blocker.
