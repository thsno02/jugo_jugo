...
edited_time: 2026-05-28T15:38:00+08:00
edited_entity: llm
...
- 这条 ablation 解释了为什么 UKPA 在 Table 4 里只动 0.033–0.045% 的词就能让 QA 腰斩——它在每个被选中的 chunk 内**也**只动 1–3 个 token。"修改总量小"与"每处修改小"是同一原则的两个尺度。这也解释了 in-place edit 家族[^v3-2] 整体为何在 PPL signal 下隐身。
- 对防御侧，这给出了一个微弱但具体的检测线索：**对 trusted snapshot 做 token-level diff，若改动落在代词/指代上但语义相近，应警惕**。可惜既有的文本层防御没有专门检测共指 token 的能力（参见文本侧防御失明[^v3-3]）。
- 对攻击者复现这套攻击的"超参选择"：默认 $d_{\text{edit}}=3$、$(\alpha, \beta, \gamma)=(0.25, 0.25, 0.5)$ 是论文给的工程默认[^src2]；改大不会让攻击更强，只会让攻击更可疑。
...
- 这条结论**只适用于 UKPA**。TKPA[^v3-4] 的等价 ablation 是"top-k 改写多少个 chunk"——论文显示 $k=3$ 时 ASR 已达 91.2%、再增大也平台化。

## Footnotes

[^src1]: `data/raw/arxiv/arxiv-graph-poisoning/agent_source_bundle.txt` — 行 817-820 — "while small edits (distance ≤ 3) already cause a drastic drop in QA accuracy (from 0.95 to 0.50), larger edits increase perplexity significantly without further improving attack impact. This results justifies the constraint on small edit distances to ensure stealthiness."
[^src2]: `data/raw/arxiv/arxiv-graph-poisoning/agent_source_bundle.txt` — 行 576-577 — 默认 $(\alpha,\beta,\gamma)=(0.25,0.25,0.5)$，γ=0.5 即明显偏向"语义接近度"以维持小编辑；同段 L815-816 等权 vs 默认权重 QA 数字 (0.55 vs 0.50)
[^v3-1]: [ukpa-coreference-disruption](ukpa-coreference-disruption.md) — UKPA 攻击机制本身
[^v3-2]: [gragpoison-additive-vs-edit-attack](gragpoison-additive-vs-edit-attack.md) — in-place edit 家族整体的 PPL 隐身性质
[^v3-3]: [graphrag-text-defense-blind-spot](graphrag-text-defense-blind-spot.md) — 文本侧防御缺乏共指 token 维度
[^v3-4]: [tkpa-graph-guided-targeted-poisoning](tkpa-graph-guided-targeted-poisoning.md) — TKPA 的等价 top-k ablation
