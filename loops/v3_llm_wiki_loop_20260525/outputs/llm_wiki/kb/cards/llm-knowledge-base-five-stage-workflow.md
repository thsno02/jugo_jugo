...
- **IDE（编辑器）。** 作者在 Obsidian 里同时打开三层视图：原始数据、编译出的 wiki、衍生的可视化。IDE 是人的"读"界面；几乎所有的"写"都是 LLM 完成的。
- **Q&A（问答）。** 当 wiki 大到一定规模（示例：~100 篇文章 / ~400K 词）时，LLM 通过读取它自己维护的索引文件和每篇文档的简短摘要来回答复杂问题；在这种"小规模"下，这套机制替代了一个独立的 RAG 层。
- **Output（输出）。** 答案不是终端里的纯文本，而是被渲染成 markdown 文件、Marp 幻灯或 matplotlib 图片，再回到 Obsidian 里查看。输出本身就是产物。
