...
- 在 NQ / HotpotQA / WoW / FEVER / MultiRC / ReCoRD 六个数据集上，ARES 平均 Kendall's τ：context relevance 0.065 高于 RAGAS、answer relevance 0.132 高于 RAGAS（见 `ares-three-judge-rag-evaluation`）。
- 与 "sampled annotations baseline"（每个 mock RAG 抽 150 条共 1,350 条人工标注）对比：ARES τ 仍高 0.08，**且仅用 78% 更少标注**。
...
- **此方法对其它 RAG 评估器同样适用**：RAGAS、GPT-3.5 judge、人工 sampled annotations 都在论文中用同一 mock 框架被对比——ARES 论文实际**也是**第一个把 mock-RAG-with-known-accuracy 当作 RAG 评估器的"meta-benchmark"。
...
- §Datasets 中 mock 构造方法：`data/raw/arxiv/arxiv-ares/agent_source_bundle.txt` 第 541–560 行。
- 正例 / 负例采样策略：第 549–553 行。
- 9 个 splits 描述与 2.5% 梯度：第 556–560 行。
- Kendall's τ 作为指标的理由：第 562–576 行。
- 与 sampled annotations baseline 的对比：第 819–822 行。

## Footnotes

[^1]: 构造意图原文（第 541–547 行）：
...
[^3]: 与 sampled annotations 对比原文（第 819–822 行）：
