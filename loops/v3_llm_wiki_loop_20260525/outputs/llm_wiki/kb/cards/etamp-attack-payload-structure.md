...
Zou 等（2026）把 eTAMP 攻击 payload 拆为三个**必须同时存在**的组件，对应 prompt injection 的三个独立失效点：
...
3. **Attack Goal（攻击目标）**：具体恶意动作，论文里统一用 `goto [URL]` 形式触发服务端动作；电商场景下是 `/checkout/add?product={ID}&express=true`（一键下单），Reddit 场景下是 `/product/{ID}?review=...rating=5`（伪造 5 星评论）。失效点：模型对单步 URL 跳转动作的工具调用。
...
- **Strategy 3 Frustration Exploitation**：trigger 升级为读 agent 的失败信号（clicks not responding / typing returns unexpected results / repeated actions not working），把恶意指令包装成"脱困办法"。
...
- Payload 三段式定义：`agent_source_bundle.txt:180-188`。
- 三种策略 payload 全文：`agent_source_bundle.txt:156-178`。

## Footnotes

- 三段式原文：`agent_source_bundle.txt:181-188` —— "Each attack payload consists of three components: Importance Signal... Trigger Condition... Attack Goal..."。
- 电商 goal 形式片段：`agent_source_bundle.txt:187` —— `/checkout/add?product={ID}&express=true`。
- Reddit goal 形式片段：`agent_source_bundle.txt:187` —— `/product/{ID}?review=...rating=5`。
