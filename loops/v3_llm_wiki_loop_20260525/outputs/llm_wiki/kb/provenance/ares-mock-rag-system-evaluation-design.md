...
card: ../cards/ares-mock-rag-system-evaluation-design.md
material_id: arxiv-ares
digest_id: digest_arxiv-ares
source_paths:
  - data/raw/arxiv/arxiv-ares/agent_source_bundle.txt
draft_card: ../../drafts/cards/ares-mock-rag-system-evaluation-design.md
draft_provenance: ../../drafts/provenance/ares-mock-rag-system-evaluation-design.md
similarity_result: ../../drafts/similarity/ares-mock-rag-system-evaluation-design.json
comparison_provenance: ../../drafts/comparison/ares-mock-rag-system-evaluation-design.md
gate:
  type: publication_gate
  result: passed
  decided_at: 2026-05-27T14:22:00+08:00
  gate_notes: 6/6 通过；9-split 2.5% 梯度构造方法 + Kendall τ 设计动机 + sampled annotations 基线对比全部锁到原文行号。
...
- 类型：publication_gate
- 结果：passed
- 决定时间：2026-05-27T14:22:00+08:00
- 检查要点：
  - 不是标题复述：完整构造方法 + 三条可信性 + 实际结果 + 四条操作含义 + 三条边界。
  - 知识密度足够：方法 + 数字（2.5% 梯度 / 9 splits / 78% 标注节约）+ 操作规则 + 边界。
  - 源支撑齐全：每条主张锁到 `agent_source_bundle.txt` 具体行号。
  - References 与 Footnotes 双章节存在。
  - frontmatter 完整合法，operational_rule 类型与正文一致。
  - related 已链 ARES 系列、ragas、graphrag。

## 备注

- 这条 evaluation methodology 可外推到 RAG / LLM judge 类研究的元评估——任何新的 RAG 评估器都应先在 mock-with-known-accuracy 上验证排序能力。
- 与 ETAMP `etamp-pseudo-trajectory-methodology` 卡片有方法学呼应：都用"构造已知 ground truth 的实验设定"来 isolate variables。

## 相关 v3 draft 工件

- draft card: `../../drafts/cards/ares-mock-rag-system-evaluation-design.md`
- draft provenance: `../../drafts/provenance/ares-mock-rag-system-evaluation-design.md`
- similarity: `../../drafts/similarity/ares-mock-rag-system-evaluation-design.json`
- comparison provenance: `../../drafts/comparison/ares-mock-rag-system-evaluation-design.md`
