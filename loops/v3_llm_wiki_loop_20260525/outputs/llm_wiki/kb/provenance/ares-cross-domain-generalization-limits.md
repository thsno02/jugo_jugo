...
card: ../cards/ares-cross-domain-generalization-limits.md
material_id: arxiv-ares
digest_id: digest_arxiv-ares
source_paths:
  - data/raw/arxiv/arxiv-ares/agent_source_bundle.txt
draft_card: ../../drafts/cards/ares-cross-domain-generalization-limits.md
draft_provenance: ../../drafts/provenance/ares-cross-domain-generalization-limits.md
similarity_result: ../../drafts/similarity/ares-cross-domain-generalization-limits.json
comparison_provenance: ../../drafts/comparison/ares-cross-domain-generalization-limits.md
gate:
  type: publication_gate
  result: passed
  decided_at: 2026-05-27T14:18:00+08:00
  gate_notes: 6/6 通过；6 个迁移对 τ 表 + 3 个剧烈漂移 τ 数字 + PPI 缓冲机制论述全部锁到原文行号。
...
- 类型：publication_gate
- 结果：passed
- 决定时间：2026-05-27T14:18:00+08:00
- 检查要点：
  - 不是标题复述：含 6 个迁移对完整 τ 表 + 三类剧烈漂移失败 + 4 条操作含义 + 3 条边界。
  - 知识密度足够：表 + 数字 + 机制（PPI 缓冲）+ 反例 + 操作规则。
  - 源支撑齐全：所有 τ 数字 / 段落锁到 `agent_source_bundle.txt` 具体行号。
  - References 与 Footnotes 双章节存在。
  - frontmatter 完整合法，distinction 类型与正文一致。
  - related 已链 ARES 系列、ragchecker、wicer。

## 备注

- 与 `ares-three-judge-rag-evaluation` 卡片有信息重叠（后者也提了剧烈漂移的 τ 数字）——本卡是该信息的专卡展开。
- 与 `ares-ppi-confidence-bound` 互补：那张卡讲 PPI 如何工作，本卡讲 PPI 在跨域下的缓冲极限。

## 相关 v3 draft 工件

- draft card: `../../drafts/cards/ares-cross-domain-generalization-limits.md`
- draft provenance: `../../drafts/provenance/ares-cross-domain-generalization-limits.md`
- similarity: `../../drafts/similarity/ares-cross-domain-generalization-limits.json`
- comparison provenance: `../../drafts/comparison/ares-cross-domain-generalization-limits.md`
