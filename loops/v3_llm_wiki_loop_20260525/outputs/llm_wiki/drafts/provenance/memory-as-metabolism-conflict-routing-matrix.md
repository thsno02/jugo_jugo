...
- 与 `memory-as-metabolism-mirror-vs-compensate` 卡片关系：那张卡讲原则与三时间尺度概要，本卡讲矩阵的**具体 7 行**——前者是 why，本卡是 how。可双向 link。
- 与 `memory-as-metabolism-architectural-separability` 强耦合（Row 7 实施依赖 separability）。Row 6 与 `audit-by-suspension-against-entrenchment` 强耦合（gravity-reduction 通路）。这些应在 wiki 内做 cross-reference。
