...
node_id:: 20260524_050036_dynamic_retrieval_as_controlled_fallback
version:: 1.0

## 为什么存在这个版本

这个版本用于把 `动态检索是受控 fallback，不是临时补料` 固化为一个可引用、可审计、可修订的 KB node。它不是最终百科条目，而是初始化阶段的 adopted knowledge object。
...
- nodes/20260524_050032_current_kb_initialization_loop/versions/1.0/card.md
- data/manifests/sources.jsonl

### 动态检索

本版本没有执行外部 retrieval；它记录 retrieval policy 和 evidence gap。实际动态检索在后续 enterprise-scale node 中执行。
...
本 node 只记录 retrieval policy 和 evidence gap，不声称已恢复被 blocked 的 Reddit 或 enterprise evidence。
