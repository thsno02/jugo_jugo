...
- `candidate_id`: `候选 3`
- `candidate_statement`: 这条帖子把 `idea file` 的理念表述为：在 LLM agents 时代，相比分享具体代码或应用，分享想法本身即可让他人的 agent 按其需求定制和构建。
- `candidate_fact_type`: `known_fact`
- `candidate_scope`: 仅限该发布帖对 `idea file` 概念的表述。
- `candidate_source_evidence`: `data/raw/webpage/karpathy-x-launch-post/raw.json`, JSON pointer `$.tweet.text`

选择理由：候选 3 来自 Karpathy 发布帖正文，事实边界清楚，能够被一个 JSON 字段直接支撑；它不同于已采纳的 LLM Wiki 模式卡，重点是 `idea file` 在 LLM agent 时代的分享逻辑。候选 5 与已采纳卡片“LLM Wiki 作为模式文件”存在明显重叠，因此本轮不选。选择不基于主题覆盖、hub、cluster 或补齐叙事结构。
...
- `llm_wiki/loop/iterations/iteration_20260525_0052_card_drafting_idea_file_agent_builds/loop_status.md`
- `llm_wiki/loop/iterations/iteration_20260525_0052_card_drafting_idea_file_agent_builds/loop_delivery.md`
- `llm_wiki/loop/iterations/iteration_20260525_0052_card_drafting_idea_file_agent_builds/read_log.md`
